...
protected:
  ...
  */
  ...
  Flow *getFlow(Mac *srcMac, Mac *dstMac, VLANid vlan_id,
                u_int16_t observation_domain_id, u_int32_t deviceIP,
  ...
  int sortHosts(u_int32_t *begin_slot, bool walk_all,
                ...
                VLANid vlan_id, OSType osFilter, u_int32_t asnFilter,
                int16_t networkFilter, u_int16_t pool_filter,
                bool filtered_hosts, bool blacklisted_hosts,
                ...
                u_int32_t device_ip, char *sortColumn);
  ...
  int sortFlows(u_int32_t *begin_slot, bool walk_all,
                ...
                Host *host, Paginator *p, const char *sortColumn);
...
public:
  ...
  void updateBroadcastDomains(VLANid vlan_id, const u_int8_t *src_mac,
  ...
  inline bool are_ip_reassignment_alerts_enabled() {
    return (enable_ip_reassignment_alerts);
  };
  ...
  virtual bool set_packet_filter([[maybe_unused]] char *filter) {
    return (false);
  };
  ...
  */
  ...
  bool getHostMinInfo(lua_State *vm, AddressTree *allowed_hosts, char *host_ip,
                      VLANid vlan_id, bool only_ndpi_stats);
  ...
  void findFlowHosts(VLANid vlan_id, u_int16_t observation_domain_id,
                     Mac *src_mac, IpAddress *_src_ip, Host **src, Mac *dst_mac,
                     IpAddress *_dst_ip, Host **dst);
  virtual Flow *findFlowByKeyAndHashId(u_int32_t key, u_int hash_id,
                                       AddressTree *allowed_hosts);
  virtual Flow *findFlowByTuple(VLANid vlan_id, u_int16_t observation_domain_id,
  ...
  void getActiveFlowsStats(nDPIStats *stats, FlowStats *status_stats,
                           AddressTree *allowed_hosts, Host *h, Paginator *p,
                           lua_State *vm, bool only_traffic_stats);
  ...
  void luaPeriodicityMap(lua_State *vm, const u_int8_t *const mac,
                         IpAddress *ip_address, VLANid vlan_id,
                         u_int16_t host_pool_id, bool unicast,
                         u_int32_t first_seen, u_int16_t filter_ndpi_proto,
                         u_int32_t maxHits);
  void luaServiceMap(lua_State *vm, const u_int8_t *const mac,
                     IpAddress *ip_address, VLANid vlan_id,
                     u_int16_t host_pool_id, bool unicast, u_int32_t first_seen,
  ...
  int getActiveHostsList(lua_State *vm, u_int32_t *begin_slot, bool walk_all,
                         u_int8_t bridge_iface_idx, AddressTree *allowed_hosts,
                         bool host_details, LocationPolicy location,
                         char *countryFilter, char *mac_filter, VLANid vlan_id,
                         OSType osFilter, u_int32_t asnFilter,
                         int16_t networkFilter, u_int16_t pool_filter,
                         bool filtered_hosts, bool blacklisted_hosts,
                         bool hide_top_hidden, u_int8_t ipver_filter,
                         int proto_filter, TrafficType traffic_type_filter,
                         u_int32_t device_ip, bool tsLua, bool anomalousOnly,
                         bool dhcpOnly, const AddressTree *const cidr_filter,
                         char *sortColumn, u_int32_t maxHits, u_int32_t toSkip,
                         bool a2zSortOrder);
  ...
  virtual Mac *getMac(u_int8_t _mac[6], bool create_if_not_present,
                      bool is_inline_call);
  virtual Host *getHost(char *host_ip, VLANid vlan_id,
  ...
  inline void setBridgeLanInterfaceId(u_int32_t v) {
    bridge_lan_interface_id = v;
  };
  inline u_int32_t getBridgeLanInterfaceId() {
    return (bridge_lan_interface_id);
  };
  inline void setBridgeWanInterfaceId(u_int32_t v) {
    bridge_wan_interface_id = v;
  };
  inline u_int32_t getBridgeWanInterfaceId() {
    return (bridge_wan_interface_id);
  };
  ...
  inline void incNumHosts(bool local) {
    if (local)
      numLocalHosts++;
    numHosts++;
  };
  inline void decNumHosts(bool local) {
    if (local)
      numLocalHosts--;
    numHosts--;
  };
  ...
  inline void profiling_section_enter(const char *label, int id) {
    PROFILING_SECTION_ENTER(label, id);
  };
  ...
  virtual u_int64_t
  getNumActiveAlertedFlows(AlertLevelGroup alert_level_group) const;
  ...
  void processExternalAlertable(AlertEntity entity, const char *entity_val,
                                bool create_if_missing, lua_State *vm,
                                u_int vm_argument_idx, bool do_store_alert);
