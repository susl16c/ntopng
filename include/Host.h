...
#endif
  u_int8_t hidden_from_top : 1, is_in_broadcast_domain : 1, is_dhcp_host : 1,
      is_crawler_bot_scanner : 1, _notused : 4;
...
public:
  ...
  Host(NetworkInterface *_iface, Mac *_mac, VLANid _vlanId,
  ...
  */
  ...
  inline VLANid get_raw_vlan_id() const {
    return (vlan_id);
  }; /* vlanId + observationPointId */
  ...
  inline void incSentTcp(u_int32_t ooo_pkts, u_int32_t retr_pkts,
    ...
    if (ooo_pkts)
      stats->incOOOSent(ooo_pkts);
    if (retr_pkts)
      stats->incRetxSent(retr_pkts);
    if (lost_pkts)
      stats->incLostSent(lost_pkts);
    if (keep_alive_pkts)
      stats->incKeepAliveSent(keep_alive_pkts);
  }

  inline void incRcvdTcp(u_int32_t ooo_pkts, u_int32_t retr_pkts,
                         u_int32_t lost_pkts, u_int32_t keep_alive_pkts) {
    if (ooo_pkts)
      stats->incOOORcvd(ooo_pkts);
    if (retr_pkts)
      stats->incRetxRcvd(retr_pkts);
    if (lost_pkts)
      stats->incLostRcvd(lost_pkts);
    if (keep_alive_pkts)
      stats->incKeepAliveRcvd(keep_alive_pkts);
  }
  ...
  inline void
  incQuotaEnforcementCategoryStats(u_int32_t when,
  ...
  inline void updateRoundTripTime(u_int32_t rtt_msecs) {
    if (as)
      as->updateRoundTripTime(rtt_msecs);
  }
  ...
  inline void reset_icmp_flood_hits() {
    if (icmp_flood.victim_counter)
      icmp_flood.victim_counter->reset_hits();
    if (icmp_flood.attacker_counter)
      icmp_flood.attacker_counter->reset_hits();
  };
  ...
  inline void reset_syn_flood_hits() {
    if (syn_flood.victim_counter)
      syn_flood.victim_counter->reset_hits();
    if (syn_flood.attacker_counter)
      syn_flood.attacker_counter->reset_hits();
  };
  ...
  inline void reset_flow_flood_hits() {
    if (flow_flood.victim_counter)
      flow_flood.victim_counter->reset_hits();
    if (flow_flood.attacker_counter)
      flow_flood.attacker_counter->reset_hits();
  };
  ...
  inline void incNumAlertedFlows(bool as_client) {
    ...
    if (stats)
      stats->incNumAlertedFlows(as_client);
  }
  ...
  inline void incNumUnreachableFlows(bool as_server) {
    if (stats)
      stats->incNumUnreachableFlows(as_server);
  }
  inline void incNumHostUnreachableFlows(bool as_server) {
    if (stats)
      stats->incNumHostUnreachableFlows(as_server);
  };
  inline void incnDPIFlows(u_int16_t l7_protocol) {
    if (stats)
      stats->incnDPIFlows(l7_protocol);
  }
  ...
  inline void reloadHideFromTop() {
    hidden_from_top = iface->isHiddenFromTop(this) ? 1 : 0;
  }
