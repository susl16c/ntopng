...
public:
  ...
  bool changeUserPermission(const char *username,
  ...
  bool existsUserLocal(const char *user) {
    char val[64];
    return getUserPasswordHashLocal(user, val);
  }
  void purgeLoopBody();

  /* Local network address list methods */
  inline u_int8_t getNumLocalNetworks() {
    return local_network_tree.getNumAddresses();
  };
  inline const char *getLocalNetworkName(int16_t local_network_id) {
    return (((u_int8_t)local_network_id < local_network_tree.getNumAddresses())
                ? local_network_names[(u_int8_t)local_network_id]
                : NULL);
  };
