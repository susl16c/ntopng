...
private:
  ...
  /* The bitmap of all possible flow alerts set by FlowCheck subclasses. When no
     alert is set, the flow is in flow_alert_normal.
  ...
  */
  ...
  u_int hash_entry_id; /* Uniquely identify this Flow inside the flows_hash hash
                          table */

  bool detection_completed, extra_dissection_completed, twh_over, twh_ok,
      dissect_next_http_packet, passVerdict, l7_protocol_guessed,
      flow_dropped_counts_increased, good_tls_hs, quota_exceeded,
      has_malicious_cli_signature, has_malicious_srv_signature, swap_done,
      swap_requested;
  ...
  time_t doNotExpireBefore; /*
                              Used for collected flows via ZMQ to make sure that
                              they are not immediately expired if their last
                              seen time is back in time with respect to ntopng
                            */
  ...
  void
  processDetectedProtocol(u_int8_t *payload,
                          u_int16_t payload_len); /* nDPI detected protocol */
...
public:
  ...
  void processIEC60870Packet(bool src2dst_direction, const u_char *ip_packet,
  ...
  inline bool isLocalToRemote() const {
    int16_t network_id;
    return get_cli_ip_addr()->isLocalHost(&network_id) &&
           !get_srv_ip_addr()->isLocalHost(&network_id);
  };
  ...
  inline const IpAddress *get_dns_srv_ip_addr() const {
    return ((get_cli_port() == 53) ? get_cli_ip_addr() : get_srv_ip_addr());
  };
  inline const IpAddress *get_dhcp_srv_ip_addr() const {
  ...
  static u_int32_t key(Host *cli, u_int16_t cli_port, Host *srv,
                       u_int16_t srv_port, VLANid vlan_id,
  ...
  void updateInterfaceLocalStats(bool src2dst_direction, u_int num_pkts,
                                 u_int pkt_len);
  ...
  inline const ContainerInfo *getClientContainerInfo() const {
    return cli_ebpf && cli_ebpf->container_info_set ? &cli_ebpf->container_info
                                                    : NULL;
  }
  inline const ContainerInfo *getServerContainerInfo() const {
    return srv_ebpf && srv_ebpf->container_info_set ? &srv_ebpf->container_info
                                                    : NULL;
  }
  inline const ProcessInfo *getClientProcessInfo() const {
    return cli_ebpf && cli_ebpf->process_info_set ? &cli_ebpf->process_info
                                                  : NULL;
  }
  inline const ProcessInfo *getServerProcessInfo() const {
    return srv_ebpf && srv_ebpf->process_info_set ? &srv_ebpf->process_info
                                                  : NULL;
  }
  ...
  inline float getEntropy(bool src2dst_direction) {
    struct ndpi_analyze_struct *e =
        src2dst_direction ? entropy.c2s : entropy.s2c;
