...
private:
  ...
  bool dump_flows_on_es, dump_flows_on_mysql, dump_flows_on_syslog,
      dump_flows_on_nindex, dump_json_flows_on_disk,
      load_json_flows_from_disk_to_nindex, dump_ext_json;
#ifdef NTOPNG_PRO
...
public:
  ...
  inline bool do_load_json_flows_from_disk_to_nindex() {
    return (load_json_flows_from_disk_to_nindex);
  };
  inline bool do_dump_flows() const {
    return (dump_flows_on_es || dump_flows_on_mysql || dump_flows_on_syslog ||
            dump_flows_on_nindex);
  };
  ...
  inline bool is_flows_dump_enabled() const {
  ...
  inline char *get_lan_interface() {
    return (lan_interface ? lan_interface : (char *)"");
  };
  inline char *get_wan_interface() {
    return (wan_interface ? wan_interface : (char *)"");
  };
  inline void set_lan_interface(char *iface) {
    if (lan_interface)
      free(lan_interface);
    lan_interface = strdup(iface);
  };
  inline void set_wan_interface(char *iface) {
    if (wan_interface)
      free(wan_interface);
    wan_interface = strdup(iface);
  };
  ...
  inline bool is_idle_local_host_cache_enabled() {
    return (enable_idle_local_hosts_cache);
  };
  ...
  TsDriver getTimeseriesDriver() const { return (timeseries_driver); }
