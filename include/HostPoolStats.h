...
public:
  ...
  inline void incStats(time_t when, u_int16_t ndpi_proto,
    ...
    if (sent_packets || rcvd_packets) {
      ...
      if (totalStats) {
        /* Unknown protocol is the only (dummy) entry used to keep track of
         * cross protocol statistics */
        totalStats->incStats(when, NDPI_PROTOCOL_UNKNOWN, sent_packets,
  ...
  inline void incCategoryStats(time_t when,
