...
void ScoreStats::lua_breakdown(lua_State *vm, bool as_client) {
  ...
  if (total == 0)
    total = 1; /* Prevents zero-division errors */
  ...
  lua_pushstring(vm, as_client ? "score_breakdown_client"
                               : "score_breakdown_server");
  lua_insert(vm, -2);
  lua_settable(vm, -3);
}
...
void ScoreStats::serialize_breakdown(ndpi_serializer *serializer) {
  ...
  for (int i = 0; i < MAX_NUM_SCORE_CATEGORIES; i++) {
    ...
    ndpi_serialize_string_uint64(serializer, score_cat.c_str(),
                                 getClient(score_category) / (float)total *
                                     100);
  }
  ...
  for (int i = 0; i < MAX_NUM_SCORE_CATEGORIES; i++) {
    ...
    ndpi_serialize_string_uint64(serializer, score_cat.c_str(),
                                 getServer(score_category) / (float)total *
                                     100);
  }
}
