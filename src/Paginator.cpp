...
Paginator::~Paginator() {
  ...
  if (host_filter)
    free(host_filter);
  if (container_filter)
    free(container_filter);
  if (pod_filter)
    free(pod_filter);
  if (mac_filter)
    free(mac_filter);
  if (traffic_profile_filter)
    free(traffic_profile_filter);
  if (username_filter)
    free(username_filter);
  if (pidname_filter)
    free(pidname_filter);
}
  ...
  while (lua_next(L, index) != 0) {
    ...
    case LUA_TSTRING:
      if (!strcmp(key, "sortColumn")) {
        ...
        // ntop->getTrace()->traceEvent(TRACE_ERROR, "Invalid string type (%s)
        // for option %s", lua_tostring(L, -1), key);
      break;
    ...
    case LUA_TBOOLEAN:
      ...
      // else
      // ntop->getTrace()->traceEvent(TRACE_ERROR, "Invalid bool type for option
      // %s", key);
      break;
