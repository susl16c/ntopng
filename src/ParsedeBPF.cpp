...
ParsedeBPF::ParsedeBPF() {
  ...
  server_info = false, free_memory = false;
}
/* *************************************** */
...
ParsedeBPF::ParsedeBPF(const ParsedeBPF &pe) {
  ...
  if (pe.ifname)
    ifname = strdup(pe.ifname);

  if ((process_info_set = pe.process_info_set)) {
    if (process_info.process_name)
      process_info.process_name = strdup(process_info.process_name);
    if (process_info.uid_name)
      process_info.uid_name = strdup(process_info.uid_name);
    if (process_info.father_process_name)
      process_info.father_process_name =
          strdup(process_info.father_process_name);
    if (process_info.father_uid_name)
      process_info.father_uid_name = strdup(process_info.father_uid_name);
  }

  if ((container_info_set = pe.container_info_set)) {
    if (container_info.id)
      container_info.id = strdup(container_info.id);
    if (container_info.name)
      container_info.name = strdup(container_info.name);

    if (container_info.data_type == container_info_data_type_k8s) {
      if (container_info.data.k8s.pod)
        container_info.data.k8s.pod = strdup(container_info.data.k8s.pod);
      if (container_info.data.k8s.ns)
        container_info.data.k8s.ns = strdup(container_info.data.k8s.ns);
...
ParsedeBPF::~ParsedeBPF() {
  if (free_memory) {
    ...
    if (process_info_set) {
      ...
      if (process_info.uid_name)
        free(process_info.uid_name);
      if (process_info.father_process_name)
        free(process_info.father_process_name);
      if (process_info.father_uid_name)
        free(process_info.father_uid_name);
    }

    if (container_info_set) {
      if (container_info.id)
        free(container_info.id);
      if (container_info.name)
        free(container_info.name);

      if (container_info.data_type == container_info_data_type_k8s) {
        if (container_info.data.k8s.pod)
          free(container_info.data.k8s.pod);
        if (container_info.data.k8s.ns)
          free(container_info.data.k8s.ns);
      } else if (container_info.data_type == container_info_data_type_docker)
        ;
    }
...
bool ParsedeBPF::update(const ParsedeBPF *const pe) {
  /* Update tcp stats */
  if (pe) {
    if (container_info_set && pe->container_info_set && container_info.id &&
      ...
      if (!warning_shown) {
        ntop->getTrace()->traceEvent(TRACE_WARNING,
                                     "Attempting to update container %s using "
                                     "information from container %s.",
...
void ParsedeBPF::print() {
  ntop->getTrace()->traceEvent(TRACE_NORMAL, "[event_type: %s]",
                               Utils::eBPFEvent2EventStr(event_type));
}
...
void ParsedeBPF::getJSONObject(json_object *my_object, bool client) const {
  ...
  if (proc_object && process_info_set && (proc = &process_info) &&
      proc->pid > 0) {
    json_object_object_add(proc_object, "PID",
                           json_object_new_int64(proc->pid));
    json_object_object_add(proc_object, "NAME",
                           json_object_new_string(proc->process_name));
    json_object_object_add(proc_object, "UID",
                           json_object_new_int64(proc->uid));
    json_object_object_add(proc_object, "GID",
                           json_object_new_int64(proc->gid));
    json_object_object_add(proc_object, "ACTUAL_MEMORY",
                           json_object_new_int64(proc->actual_memory));
    json_object_object_add(proc_object, "PEAK_MEMORY",
                           json_object_new_int64(proc->peak_memory));
    json_object_object_add(proc_object, "USER_NAME",
                           json_object_new_string(proc->uid_name));

    if (proc->father_pid > 0) {
      json_object_object_add(proc_object, "FATHER_PID",
                             json_object_new_int64(proc->father_pid));
      json_object_object_add(proc_object, "FATHER_NAME",
                             json_object_new_string(proc->father_process_name));
      json_object_object_add(proc_object, "FATHER_UID",
                             json_object_new_int64(proc->father_uid));
      json_object_object_add(proc_object, "FATHER_GID",
                             json_object_new_int64(proc->father_gid));
      json_object_object_add(proc_object, "FATHER_USER_NAME",
                             json_object_new_string(proc->father_uid_name));
    }

    json_object_object_add(
        my_object, client ? "CLIENT_PROCESS" : "SERVER_PROCESS", proc_object);
  }

  if (cont_object && container_info_set && (cont = &container_info)) {
    if (cont->id)
      json_object_object_add(cont_object, "ID",
                             json_object_new_string(cont->id));

    if (cont->data_type == container_info_data_type_k8s) {
      if (cont->name)
        json_object_object_add(cont_object, "K8S_NAME",
                               json_object_new_string(cont->name));
      if (cont->data.k8s.pod)
        json_object_object_add(cont_object, "K8S_POD",
                               json_object_new_string(cont->data.k8s.pod));
      if (cont->data.k8s.ns)
        json_object_object_add(cont_object, "K8S_NS",
                               json_object_new_string(cont->data.k8s.ns));
    } else if (cont->data_type == container_info_data_type_docker) {
      if (cont->name)
        json_object_object_add(cont_object, "DOCKER_NAME",
                               json_object_new_string(cont->name));
    }

    json_object_object_add(my_object,
                           client ? "CLIENT_CONTAINER" : "SERVER_CONTAINER",
                           cont_object);
  }
  ...
  if (tcp_object && tcp_info_set && (tcp = &tcp_info)) {
    ...
    json_object_object_add(
        my_object, client ? "CLIENT_TCP_INFO" : "SERVER_TCP_INFO", tcp_object);
  }
}
