...
ContinuousPing::~ContinuousPing() {
  ...
  for (std::map<std::string, ContinuousPingStats *>::iterator it =
           v6_results.begin();
       it != v6_results.end(); ++it)
    delete it->second;

  if (started)
    pthread_join(poller, NULL);

  for (std::map<std::string, Ping *>::iterator it = if_pinger.begin();
       it != if_pinger.end(); ++it)
    delete it->second;
...
#ifdef TRACE_PING
      ntop->getTrace()->traceEvent(TRACE_NORMAL, "Adding host to ping %s",
                                   _addr);
#endif
    }
  } else {
...
#ifdef TRACE_PING
    ntop->getTrace()->traceEvent(
        TRACE_NORMAL, "Pinging host %s [last: %d][diff: %d]",
...
void ContinuousPing::readPingResults() {
  ...
  for (std::map<std::string, ContinuousPingStats *>::iterator it =
       ...
       it != v4_results.end(); ++it) {
    float f =
        it->second->getPinger()->getRTT(it->first.c_str(), false /* v6 */);

#ifdef TRACE_PING
    ntop->getTrace()->traceEvent(TRACE_NORMAL, "%s() [IPv4] %s=%f",
                                 __FUNCTION__, it->first.c_str(), f);
#endif
    ...
    if (f != -1)
      ...
      ntop->getTrace()->traceEvent(TRACE_NORMAL,
                                   "[IPv4] Missing ping response for %s",
                                   it->first.c_str());
#endif
    }
  }
...
#ifdef TRACE_PING
    ntop->getTrace()->traceEvent(TRACE_NORMAL, "%s() [IPv6] %s=%f",
                                 __FUNCTION__, it->first.c_str(), f);
#endif
...
#ifdef TRACE_PING_DROPS
      ntop->getTrace()->traceEvent(TRACE_NORMAL,
                                   "[IPv6] Missing ping response for %s",
                                   it->first.c_str());
#endif
    }
  }
...
void ContinuousPing::collectProtoResponse(
    lua_State *vm, std::map<std::string, ContinuousPingStats *> *w) {
  for (std::map<std::string, ContinuousPingStats *>::iterator it = w->begin();
       it != w->end(); ++it) {
    if (it->first.c_str()[0]) {
...
void ContinuousPing::collectResponses(lua_State *vm, bool v6) {
  ...
  for (std::map<std::string, bool>::const_iterator it = pinged->begin();
       it != pinged->end(); ++it) {
#ifdef TRACE_PING
    ntop->getTrace()->traceEvent(TRACE_WARNING, "Missing ping response for %s",
                                 it->first.c_str());
#endif
...
void ContinuousPing::cleanupInactiveHosts() {
  ...
  std::map<std::string /* IP */, ContinuousPingStats * /* stats */>::iterator
      it1;
...
#ifdef TRACE_PING
    ntop->getTrace()->traceEvent(TRACE_NORMAL, "[v4] Discarding host %s",
                                 it->c_str());
#endif
...
#ifdef TRACE_PING
    ntop->getTrace()->traceEvent(TRACE_NORMAL, "[v6] Discarding host %s",
                                 it->c_str());
#endif
