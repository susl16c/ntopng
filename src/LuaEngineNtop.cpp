...
void get_host_vlan_info(char *lua_ip, char **host_ip, VLANid *vlan_id,
...
static int ntop_msleep(lua_State *vm) {
  ...
  if (ms_duration > max_duration)
    ms_duration = max_duration;
...
static int ntop_tcp_probe(lua_State *vm) {
  ...
  if (lua_type(vm, 3) == LUA_TNUMBER)
    timeout = (u_int16_t)lua_tonumber(vm, 3);
...
static int ntop_is_pcap_download_allowed(lua_State *vm) {
  ...
  lua_pushboolean(vm,
                  ntop->isPcapDownloadAllowed(vm, ntop_interface->get_name()));
  return (ntop_lua_return_value(vm, __FUNCTION__, CONST_LUA_OK));
}
...
static int ntop_change_user_permission(lua_State *vm) {
  ...
  lua_pushboolean(vm,
                  ntop->changeUserPermission(username, allow_pcap_download));
  return CONST_LUA_OK;
}
...
static int ntop_post_http_json_data(lua_State *vm) {
  ...
  bool rv =
      Utils::postHTTPJsonData(username, password, url, json, timeout, &stats);
...
static int ntop_add_user(lua_State *vm) {
  ...
  lua_pushboolean(vm,
                  ntop->addUser(username, full_name, password, host_role,
...
static int ntop_run_extraction(lua_State *vm) {
  ...
  if (lua_tostring(vm, 7))
    timeline_path = (char *)lua_tostring(vm, 7);
...
static int ntop_run_live_extraction(lua_State *vm) {
  ...
  if ((bpf = (char *)lua_tostring(vm, 4)) == NULL)
    return (ntop_lua_return_value(vm, __FUNCTION__, CONST_LUA_PARAM_ERROR));
  if (lua_tostring(vm, 5))
    timeline_path = (char *)lua_tostring(vm, 5);
...
static int ntop_get_info(lua_State *vm) {
  ...
  snprintf(rsp, sizeof(rsp), "%s [%s][%s]", PACKAGE_OSNAME, PACKAGE_MACHINE,
           PACKAGE_OS);
...
static int ntop_snmp_max_num_engines(lua_State *vm) {
  lua_pushinteger(vm, MAX_NUM_ASYNC_SNMP_ENGINES);
...
static int ntop_check_sub_interface_syntax(lua_State *vm) {
  ...
  lua_pushboolean(vm, ntop_interface
                          ? ntop_interface->checkSubInterfaceSyntax(filter)
                          : false);
...
static int ntop_check_filter_syntax(lua_State *vm) {
  ...
  lua_pushboolean(vm, ntop_interface ? ntop_interface->checkFilterSyntax(filter)
                                     : false);
...
static int ntop_rrd_update(lua_State *vm) {
  ...
  if (lua_type(vm, 3) == LUA_TSTRING)
    v1 = (const char *)lua_tostring(vm, 3);
  if (lua_type(vm, 4) == LUA_TSTRING)
    v2 = (const char *)lua_tostring(vm, 4);
  if (lua_type(vm, 5) == LUA_TSTRING)
    v3 = (const char *)lua_tostring(vm, 5);
  if (lua_type(vm, 6) == LUA_TSTRING)
    v4 = (const char *)lua_tostring(vm, 6);
...
static int ntop_recipient_register(lua_State *vm) {
  ...
  u_int8_t enabled_categories =
      0xFF; /* MUST be large enough to contain MAX_NUM_SCRIPT_CATEGORIES */
...
static int ntop_get_ndpi_protocol_category(lua_State *vm) {
  ...
  lua_push_str_table_entry(
      vm, "name", (char *)ntop_interface->get_ndpi_category_name(category));
