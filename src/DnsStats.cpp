...
void DnsStats::incStats(bool as_client, const FlowDNSStats *fts) {
  ...
  if (fts->num_a)
    qry->breakdown.num_a += fts->num_a, tot_qry += fts->num_a;
  if (fts->num_ns)
    qry->breakdown.num_ns += fts->num_ns, tot_qry += fts->num_ns;
  if (fts->num_cname)
  ...
  if (fts->num_mx)
    qry->breakdown.num_mx += fts->num_mx, tot_qry += fts->num_mx;
  if (fts->num_txt)
  ...
  if (tot_qry)
    qry->num_queries.inc(tot_qry);

  if (fts->num_replies_ok)
    rsp->num_replies_ok.inc(fts->num_replies_ok);
