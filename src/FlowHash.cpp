...
Flow *FlowHash::find(IpAddress *src_ip, IpAddress *dst_ip, u_int16_t src_port,
                     u_int16_t dst_port, VLANid vlanId,
                     u_int16_t observation_point_id, u_int8_t protocol,
  ...
  // ntop->getTrace()->traceEvent(TRACE_NORMAL, "%u:%u / %u:%u [icmp: %u][key:
  // %u][icmp info key: %u][head: 0x%x]", src_ip->key(), src_port,
  // dst_ip->key(), dst_port, icmp_info ? 1 : 0, hash, icmp_info ?
  // icmp_info->key() : 0, head);
  ...
  while (head) {
    if (!head->idle() &&
        !head->is_swap_done() /* Do NOT return flows for which swap has been
                                 done. Leave them so they will be marked as idle
                                 and disappear */
        && head->equal(src_ip, dst_ip, src_port, dst_port, vlanId,
                       observation_point_id, protocol, icmp_info,
                       src2dst_direction)) {
