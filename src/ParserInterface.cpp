...
bool ParserInterface::processFlow(ParsedFlow *zflow) {
  ...
  if (!isSubInterface()) {
    ...
    if (!processed && flowHashingMode != flowhashing_none) {
      ...
      case flowhashing_probe_ip_and_ingress_iface_idx:
        // ntop->getTrace()->traceEvent(TRACE_NORMAL, "[IP: %u][inIndex: %u]",
  ...
  flow = getFlow(srcMac, dstMac, zflow->vlan_id, zflow->observationPointId,
                 zflow->device_ip, zflow->inIndex, zflow->outIndex,
  ...
  */
  ...
  flow->updateInterfaceLocalStats(
      src2dst_direction,
      zflow->pkt_sampling_rate * (zflow->in_pkts + zflow->out_pkts),
      zflow->pkt_sampling_rate * (zflow->in_bytes + zflow->out_bytes));
  ...
  */
  ...
  if (zflow->vrfId)
    flow->setVRFid(zflow->vrfId);

  if (zflow->src_as)
    flow->setSrcAS(zflow->src_as);
  if (zflow->dst_as)
    flow->setDstAS(zflow->dst_as);
  ...
  if (zflow->ja3c_hash)
    flow->updateJA3C(zflow->ja3c_hash);
  if (zflow->ja3s_hash)
    flow->updateJA3S(zflow->ja3s_hash);
...
bool ParserInterface::isProbingFlow(const ParsedFlow *zflow) {
  ...
  case IPPROTO_TCP: {
    /* zflow->tcp.tcp_flags are, according to the specs, the 'Cumulative of all
       the TCP flags seen for this flow'. Hence, for bi-directional flows, they
       are the locigal OR of client and server flags whereas for
       mono-directional
       flows they are the logical OR of client-to-server flags. */

    /* A SYN only seen by the client is very likely a scan. Any established TCP
       connection involves at least an ACK from both parties as this is also
       part of the initial three-way-handshake. */
    ...
    /* A client SYN+RST can be found when a scan finds the destination port
       ...
       2. server responds with SYN+ACK as its port 22 is open and it is willing
