...
static int ntop_interface_live_capture(lua_State *vm) {
  ...
  if (lua_type(vm, 1) == LUA_TSTRING) /* Host */ {
    ...
    get_host_vlan_info((char *)lua_tostring(vm, 1), &key, &vlan_id, host_ip,
                       sizeof(host_ip));
...
static int ntop_interface_exec_sql_query(lua_State *vm) {
  ...
  if (!ntop_interface)
    ...
    if (ntop_interface->exec_sql_query(vm, sql, limit_rows,
                                       wait_for_db_created) < 0)
      lua_pushnil(vm);
...
static int ntop_interface_store_external_alert(lua_State *vm) {
  ...
  iface->processExternalAlertable(entity, entity_value,
                                  true /* Create if not exists */, vm, idx,
                                  true /* store alert */);
...
static int ntop_interface_release_external_alert(lua_State *vm) {
  ...
  iface->processExternalAlertable(entity, entity_value,
                                  false /* Don't create if not exists */, vm,
                                  idx, false /* release alert */);
...
static int ntop_interface_alert_store_query(lua_State *vm) {
  ...
  if (!iface || lua_type(vm, 1) != LUA_TSTRING ||
...
static int ntop_get_batched_interface_hosts(lua_State *vm,
  ...
  if (lua_type(vm, 3) == LUA_TNUMBER)
    maxHits = (u_int32_t)lua_tonumber(vm, 3);
  if (lua_type(vm, 4) == LUA_TBOOLEAN)
    anomalousOnly = lua_toboolean(vm, 4);
  /* If parameter 5 is true, the caller wants to iterate all hosts, including
     those with unidirectional traffic.
     If parameter 5 is false, then the caller only wants host withs
  ...
  if ((!ntop_interface) ||
      ntop_interface->getActiveHostsList(
          ...
          filtered_hosts, blacklisted_hosts, hide_top_hidden, ipver_filter,
          proto_filter, traffic_type_filter, 0 /* probe ip */,
...
static int ntop_get_interface_hosts(lua_State *vm, LocationPolicy location) {
  ...
  if (lua_type(vm, 7) == LUA_TNUMBER)
    os_filter = (OSType)lua_tointeger(vm, 7);
  ...
  if (lua_type(vm, 14) == LUA_TNUMBER)
    proto_filter = (int)lua_tonumber(vm, 14);
  if (lua_type(vm, 15) == LUA_TNUMBER)
    traffic_type_filter = (TrafficType)lua_tointeger(vm, 15);
  if (lua_type(vm, 16) == LUA_TBOOLEAN)
    filtered_hosts = lua_toboolean(vm, 16);
  if (lua_type(vm, 17) == LUA_TBOOLEAN)
    blacklisted_hosts = lua_toboolean(vm, 17);
  if (lua_type(vm, 18) == LUA_TBOOLEAN)
    hide_top_hidden = lua_toboolean(vm, 18);
  if (lua_type(vm, 19) == LUA_TBOOLEAN)
    anomalousOnly = lua_toboolean(vm, 19);
  if (lua_type(vm, 20) == LUA_TBOOLEAN)
    dhcpOnly = lua_toboolean(vm, 20);
  if (lua_type(vm, 21) == LUA_TSTRING)
    cidr_filter.addAddress(lua_tostring(vm, 21)), cidr_filter_enabled = true;
  if (lua_type(vm, 22) == LUA_TSTRING)
    device_ip = ntohl(inet_addr(lua_tostring(vm, 22)));
  ...
  if ((!ntop_interface) ||
      ntop_interface->getActiveHostsList(
          ...
          vlan_filter, os_filter, asn_filter, network_filter, pool_filter,
          filtered_hosts, blacklisted_hosts, hide_top_hidden, ipver_filter,
          proto_filter, traffic_type_filter, device_ip, false /* host->lua */,
          anomalousOnly, dhcpOnly, cidr_filter_enabled ? &cidr_filter : NULL,
          sortColumn, maxHits, toSkip, a2zSortOrder) < 0)
    return (ntop_lua_return_value(vm, __FUNCTION__, CONST_LUA_ERROR));
...
static int ntop_get_interface_macs_info(lua_State *vm) {
  ...
  if (lua_type(vm, 4) == LUA_TBOOLEAN)
    a2zSortOrder = lua_toboolean(vm, 4);
  if (lua_type(vm, 5) == LUA_TBOOLEAN)
    sourceMacsOnly = lua_toboolean(vm, 5);
  if (lua_type(vm, 6) == LUA_TSTRING)
    manufacturer = lua_tostring(vm, 6);
  if (lua_type(vm, 7) == LUA_TNUMBER)
  ...
  if (lua_type(vm, 10) == LUA_TNUMBER)
    min_first_seen = lua_tonumber(vm, 10);
...
static int ntop_get_interface_flows_stats(lua_State *vm) {
  ...
  ntop->getTrace()->traceEvent(TRACE_DEBUG, "%s() called", __FUNCTION__);
  if (ntop_interface)
    ntop_interface->getFlowsStats(vm);
...
static int ntop_discover_iface_hosts(lua_State *vm) {
  ...
  if (lua_type(vm, 1) == LUA_TNUMBER)
    timeout = (u_int)lua_tonumber(vm, 1);
...
static int ntop_get_interface_flow_key(lua_State *vm) {
  ...
  if (cli_name == NULL || srv_name == NULL ||
    ...
    lua_pushinteger(vm, Flow::key(cli, cli_port, srv, srv_port, cli_vlan,
                                  getLuaVMUservalue(vm, observationPointId),
                                  protocol));
...
static int ntop_get_interface_find_flow_by_tuple(lua_State *vm) {
  ...
  f = ntop_interface->findFlowByTuple(
      vlan_id, getLuaVMUservalue(vm, observationPointId), &src_ip_addr,
      &dst_ip_addr, htons(src_port), htons(dst_port), l4_proto, ptree);
...
static int ntop_dump_local_hosts_2_redis(lua_State *vm) {
  ...
  lua_pushnil(vm);
  ntop_interface->dumpLocalHosts2redis(
      true /* must disable purge as we are called from lua */);
...
static int ntop_get_ndpi_protocols(lua_State *vm) {
  ...
  if ((lua_type(vm, 2) == LUA_TBOOLEAN))
    skip_critical = lua_toboolean(vm, 2);
...
static int ntop_get_active_flows_stats(lua_State *vm) {
  ...
  if (ntop_interface) {
    ntop_interface->getActiveFlowsStats(&ndpi_stats, &stats,
                                        get_allowed_nets(vm), host, p, vm,
                                        only_traffic_stats);
  } else
    lua_pushnil(vm);
...
static int ntop_interface_check_context(lua_State *vm) {
  ...
  if ((c->iface == NULL) ||
     ...
     * supported */
    ntop->getTrace()->traceEvent(TRACE_WARNING, "Bad context for interface %s",
                                 entity_val);
...
static int ntop_get_interface_map(lua_State *vm, bool periodicity) {
  ...
  if (lua_type(vm, 2) == LUA_TNUMBER)
    vlan_id = (u_int16_t)lua_tonumber(vm, 2);
  if (lua_type(vm, 3) == LUA_TNUMBER)
    host_pool_id = (u_int16_t)lua_tonumber(vm, 3);
  if (lua_type(vm, 4) == LUA_TBOOLEAN)
    unicast = (bool)lua_toboolean(vm, 4);
  if (lua_type(vm, 5) == LUA_TNUMBER)
    first_seen = (u_int32_t)lua_tonumber(vm, 5);
  if (lua_type(vm, 6) == LUA_TSTRING)
    l7_proto = (char *)lua_tostring(vm, 6);
  if (lua_type(vm, 7) == LUA_TNUMBER)
    maxHits = (u_int32_t)lua_tonumber(vm, 7);
  ...
  if (ntop_interface) {
    if (periodicity)
      ntop_interface->luaPeriodicityMap(vm, mac, ip, vlan_id, host_pool_id,
                                        unicast, first_seen, filter_ndpi_proto,
                                        maxHits);
    else
      ntop_interface->luaServiceMap(vm, mac, ip, vlan_id, host_pool_id, unicast,
                                    first_seen, filter_ndpi_proto, maxHits);
  } else
    lua_pushnil(vm);
...
static int ntop_interface_service_map_set_multiple_status(lua_State *vm) {
  ...
  if (ntop_interface) {
    ...
    if (lua_type(vm, 1) == LUA_TSTRING)
      l7_proto = (char *)lua_tostring(vm, 1);
    if (lua_type(vm, 2) == LUA_TNUMBER)
...
static luaL_Reg _ntop_interface_reg[] = {
    ...
    {"periodicityFilteringMenu",
     ntop_get_interface_periodicity_proto_filtering_menu},
