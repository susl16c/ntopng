...
nDPIStats::nDPIStats(bool enable_throughput_stats, bool enable_behavior_stats) {
  ...
  if (enable_behavior_stats)
    behavior_bytes_traffic =
        new (std::nothrow) BehaviorAnalysis *[MAX_NDPI_PROTOS]();
#endif
  ...
  for (int i = 0; i < MAX_NDPI_PROTOS; i++) {
    if (counters[i] != NULL) {
      if (stats->counters[i] == NULL) {
        if ((stats->counters[i] =
                 (ProtoCounter *)calloc(1, sizeof(ProtoCounter))) == NULL) {
      ...
      if (bytes_thpt && bytes_thpt[i] && stats->bytes_thpt) {
        if (!stats->bytes_thpt[i])
          stats->bytes_thpt[i] =
              new (std::nothrow) ThroughputStats(*bytes_thpt[i]);
        else
  ...
  for (int i = 0; i < MAX_NDPI_PROTOS; i++) {
    if (counters[i] != NULL) {
      if (counters[i]->bytes.sent || counters[i]->bytes.rcvd)
        printf("[%s] [pkts: %llu/%llu][bytes: %llu/%llu][duration: %u "
               "sec][thpt: %.2f]\n",
               iface->get_ndpi_proto_name(i),
               (long long unsigned)counters[i]->packets.sent,
               (long long unsigned)counters[i]->packets.rcvd,
               (long long unsigned)counters[i]->bytes.sent,
               (long long unsigned)counters[i]->bytes.rcvd,
               counters[i]->duration,
        ...
        if (counters[i]->bytes.sent || counters[i]->bytes.rcvd ||
          ...
          if (!tsLua) {
            ...
            lua_push_str_table_entry(vm, "breed",
                                     iface->get_ndpi_proto_breed_name(i));
            lua_push_uint64_table_entry(vm, "packets.sent",
                                        counters[i]->packets.sent);
            lua_push_uint64_table_entry(vm, "packets.rcvd",
                                        counters[i]->packets.rcvd);
            lua_push_uint64_table_entry(vm, "bytes.sent",
                                        counters[i]->bytes.sent);
            lua_push_uint64_table_entry(vm, "bytes.rcvd",
                                        counters[i]->bytes.rcvd);
            lua_push_uint64_table_entry(vm, "duration", counters[i]->duration);
            lua_push_uint64_table_entry(vm, "num_flows",
                                        counters[i]->total_flows);

#ifdef NTOPNG_PRO
            if (behavior_bytes_traffic && behavior_bytes_traffic[i])
              behavior_bytes_traffic[i]->luaBehavior(
            ...
            if (bytes_thpt && bytes_thpt[i]) {
              ...
              lua_push_uint64_table_entry(vm, "trend_bps",
                                          bytes_thpt[i]->getTrend());
  ...
  if (with_categories) {
    ...
    for (int i = 0; i < NDPI_PROTOCOL_NUM_CATEGORIES; i++) {
      if (cat_counters[i].bytes.sent + cat_counters[i].bytes.rcvd) {
        const char *name =
            iface->get_ndpi_category_name((ndpi_protocol_category_t)i);
        ...
        if (!tsLua) {
          ...
          lua_push_uint64_table_entry(vm, "bytes",
                                      cat_counters[i].bytes.sent +
                                          cat_counters[i].bytes.rcvd);
          lua_push_uint64_table_entry(vm, "bytes.sent",
                                      cat_counters[i].bytes.sent);
          lua_push_uint64_table_entry(vm, "bytes.rcvd",
                                      cat_counters[i].bytes.rcvd);
...
void nDPIStats::updateStats(const struct timeval *tv) {
  ...
  for (int i = 0; i < MAX_NDPI_PROTOS; i++) {
    ...
    if (bytes_thpt[i])
      bytes_thpt[i]->updateStats(tv, counters[i]->bytes.sent +
                                         counters[i]->bytes.rcvd);
    ...
    if (tv->tv_sec >= nextMinPeriodicUpdate) {
      ...
      if (!behavior_bytes_traffic[i])
        behavior_bytes_traffic[i] = new (std::nothrow) BehaviorAnalysis(
            0.9 /* Alpha parameter */, 0.1 /* Beta parameter */,
            0.05 /* Significance */, true /* Counter */);

      if (behavior_bytes_traffic[i])
        behavior_bytes_traffic[i]->updateBehavior(
            NULL, counters[i]->bytes.sent + counters[i]->bytes.rcvd, NULL,
            false);
...
void nDPIStats::incStats(u_int32_t when, u_int16_t proto_id,
  ...
  if (proto_id < (MAX_NDPI_PROTOS)) {
    ...
    counters[proto_id]->packets.sent += sent_packets,
        counters[proto_id]->bytes.sent += sent_bytes;
    counters[proto_id]->packets.rcvd += rcvd_packets,
        counters[proto_id]->bytes.rcvd += rcvd_bytes;

    if ((when != 0) && (when - counters[proto_id]->last_epoch_update >=
                        ntop->getPrefs()->get_housekeeping_frequency())) {
      counters[proto_id]->duration +=
          ntop->getPrefs()->get_housekeeping_frequency(),
...
void nDPIStats::incCategoryStats(u_int32_t when,
  ...
  if (category_id < NDPI_PROTOCOL_NUM_CATEGORIES) {
    ...
    if ((when != 0) && (when - cat_counters[category_id].last_epoch_update >=
                        ntop->getPrefs()->get_housekeeping_frequency())) {
      cat_counters[category_id].duration +=
          ntop->getPrefs()->get_housekeeping_frequency(),
...
static void addProtoJson(json_object *my_object, ProtoCounter *counter,
                         const char *name) {
  ...
  json_object_object_add(inner, "duration",
                         json_object_new_int64(counter->duration));
  ...
  json_object_object_add(inner1, "sent",
                         json_object_new_int64(counter->bytes.sent));
  json_object_object_add(inner1, "rcvd",
                         json_object_new_int64(counter->bytes.rcvd));
  ...
  json_object_object_add(inner1, "sent",
                         json_object_new_int64(counter->packets.sent));
  json_object_object_add(inner1, "rcvd",
                         json_object_new_int64(counter->packets.rcvd));
...
json_object *nDPIStats::getJSONObject(NetworkInterface *iface) {
  ...
  for (int i = 0; i < NDPI_PROTOCOL_NUM_CATEGORIES; i++) {
    if (cat_counters[i].bytes.sent + cat_counters[i].bytes.rcvd > 0) {
      ...
      json_object_object_add(inner1, "bytes_sent",
                             json_object_new_int64(cat_counters[i].bytes.sent));
      json_object_object_add(inner1, "bytes_rcvd",
                             json_object_new_int64(cat_counters[i].bytes.rcvd));
      json_object_object_add(inner1, "duration",
                             json_object_new_int64(cat_counters[i].duration));

      json_object_object_add(
          inner, iface->get_ndpi_category_name((ndpi_protocol_category_t)i),
          inner1);
    }
  }
...
void nDPIStats::resetStats() {
  for (int i = 0; i < MAX_NDPI_PROTOS; i++) {
    /* NOTE: do not deallocate counters since they can be in use by other
     * threads */
    if (counters[i] != NULL) {
