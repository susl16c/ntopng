...
ICMPstats::~ICMPstats() {
  ...
  for (it = stats.begin(); it != stats.end(); ++it) {
    if (it->second.last_host_sent_peer)
      free(it->second.last_host_sent_peer);
    if (it->second.last_host_rcvd_peer)
      free(it->second.last_host_rcvd_peer);
  }
  stats.clear();
}
...
void ICMPstats::incStats(u_int32_t num_pkts, u_int8_t icmp_type,
  ...
  if (sent) {
    ...
    if (peer) {
      if (s.last_host_sent_peer)
        free(s.last_host_sent_peer);
    ...
    if (peer) {
      if (s.last_host_rcvd_peer)
        free(s.last_host_rcvd_peer);
...
void ICMPstats::addToTable(const char *label, lua_State *vm,
  ...
  if (verbose)
    lua_push_str_table_entry(vm, "last_host_sent_peer",
                             curr->last_host_sent_peer);
  lua_push_uint64_table_entry(vm, "rcvd", curr->pkt_rcvd);
  if (verbose)
    lua_push_str_table_entry(vm, "last_host_rcvd_peer",
                             curr->last_host_rcvd_peer);
