...
InterfaceStatsHash::~InterfaceStatsHash() {
  for (u_int i = 0; i < max_hash_size; i++) {
    if (buckets[i] != NULL) {
      if (buckets[i]->ifName)
        free(buckets[i]->ifName);

      if (buckets[i]->container_info_set) {
        if (buckets[i]->container_info.id)
          free(buckets[i]->container_info.id);
...
bool InterfaceStatsHash::set(const sFlowInterfaceStats *const stats) {
  ...
  if (!buckets[hash])
      ...
      if (head->deviceIP == deviceIP && head->ifIndex == ifIndex &&
        ...
        if (num_runs >= max_hash_size) {
          ntop->getTrace()->traceEvent(TRACE_WARNING,
                                       "Internal error: too many loops=%u",
                                       max_hash_size);
    ...
    if (head) {
      ...
      if (stats->samplesGenerated > 0 &&
        ...
        /*
          ...
          Ubiquiti routers when generating sFlow send two counters samples in
          two different packets with the same value for `samplesGenerated`. The
          first counter sample has data for the IN direction and the second
          counter sample has data for the OUT direction. In this case, we need
          to check and update values rather than overwriting them.
         */
    ...
    new_bucket:
      buckets[hash] =
          (sFlowInterfaceStats *)malloc(sizeof(sFlowInterfaceStats));
      if (buckets[hash]) {
