...
Flow *ViewInterface::findFlowByTuple(VLANid vlan_id,
  ...
  for (u_int8_t s = 0; s < num_viewed_interfaces; s++) {
    if ((f = (Flow *)viewed_interfaces[s]->findFlowByTuple(
             vlan_id, observation_point_id, src_ip, dst_ip, src_port, dst_port,
             l4_proto, allowed_hosts)))
      break;
  }
  ...
  if (f->get_partial_traffic_stats_view(&partials, &first_partial)) {
    ...
    if (cli_ip && srv_ip) {
      ...
      if (first_partial) {
        ...
        if (cli_host)
          cli_host->setViewInterfaceMac(f->getViewCliMac());
        if (srv_host)
          srv_host->setViewInterfaceMac(f->getViewSrvMac());
      } else {
        /* The unsafe pointers can be used here as
         * ViewInterface::viewed_flows_walker is called synchronously with the
         * ViewInterface purgeIdle. This also saves some
...
bool ViewInterface::isSampledTraffic() const {
  for (u_int8_t s = 0; s < num_viewed_interfaces; s++)
    if (viewed_interfaces[s]->isSampledTraffic())
      return true;
