...
ZMQCollectorInterface::ZMQCollectorInterface(const char *_endpoint)
  ...
  char *tmp, *e, *t;
  const char *topics[] = {"flow",   "event", "counter",         "template",
                          "option", "hello", "listening-ports", NULL};
...
ZMQCollectorInterface::~ZMQCollectorInterface() {
  ...
  if (n > 0) {
    for (u_int i = 0; i < PROFILING_NUM_SECTIONS; i++) {
      ...
      ntop->getTrace()->traceEvent(
          TRACE_NORMAL, "[PROFILING] Section #%d '%s': %llu ticks", i,
          PROFILING_SECTION_LABEL(i), PROFILING_SECTION_TICKS(i));
    }
  }
#endif

  for (int i = 0; i < num_subscribers; i++) {
    if (subscriber[i].endpoint)
      free(subscriber[i].endpoint);
...
void ZMQCollectorInterface::collect_flows() {
  ...
  struct zmq_msg_hdr *h =
      (struct zmq_msg_hdr *)&h0; /* NOTE: in network-byte-order format */
  char *payload = NULL;
    ...
    for (int subscriber_id = 0; subscriber_id < num_subscribers;
      ...
      if (items[subscriber_id].revents & ZMQ_POLLIN) {
        ...
        if (size == sizeof(struct zmq_msg_hdr_v0)) {
          ...
          if (size != sizeof(struct zmq_msg_hdr) ||
            ...
            ntop->getTrace()->traceEvent(
                ...
                "outdated? [%u][%u][%u][%u][%u]",
                size, sizeof(struct zmq_msg_hdr), h->version, ZMQ_MSG_VERSION,
                ZMQ_COMPATIBILITY_MSG_VERSION);
        ...
        ntop->getTrace()->traceEvent(TRACE_NORMAL,
                                     "[subscriber_id: %u][message source: %u]"
                                     "[msg_id: %u][last_msg_id: %u][lost: %i]",
                                     subscriber_id, source_id, msg_id,
                                     last_msg_id, msg_id - last_msg_id - 1);
#endif
        ...
        if (msg_id > 0) {
          if (msg_id < last_msg_id)
            ...
            if (diff > 1) {
              ...
              ntop->getTrace()->traceEvent(
                  TRACE_NORMAL,
                  "[msg_id=%u][last=%u][tot_msgs=%u][drops=%u][+%u]", msg_id,
                  last_msg_id, recvStats.zmq_msg_rcvd, recvStats.zmq_msg_drops,
                  diff - 1);
#endif
            }
          }
        ...
        */
        ...
        if (size > 0 && (u_int32_t)size > payload_len)
          ...
          case 'f': /* flow */
            if (tlv_encoding)
              recvStats.num_flows += parseTLVFlow(
                  uncompressed, uncompressed_len, subscriber_id, this);
            else {
              uncompressed[uncompressed_len] = '\0';
              recvStats.num_flows +=
                  parseJSONFlow(uncompressed, uncompressed_len, subscriber_id);
            }
            break;
