...
ExportInterface::ExportInterface(const char *_endpoint, const char *_topic) {
  ...
  if (ntop->getPrefs()->get_export_zmq_encryption_key()) {
    ...
    rc = zmq_setsockopt(publisher, ZMQ_CURVE_SERVERKEY, server_public_key,
                        strlen(server_public_key) + 1);

    if (rc != 0) {
      ntop->getTrace()->traceEvent(
          TRACE_ERROR, "Error setting ZMQ_CURVE_SERVERKEY = %s (%d)",
          server_public_key, errno);
      throw("Error setting ZMQ_CURVE_SERVERKEY");
    }

    rc = zmq_setsockopt(publisher, ZMQ_CURVE_PUBLICKEY, client_public_key,
  ...
  if (zmq_bind(publisher, endpoint) != 0) {
    ntop->getTrace()->traceEvent(TRACE_ERROR,
                                 "Unable to bind ZMQ endpoint %s: %s", endpoint,
                                 strerror(errno));
    throw("Unable to bind ZMQ endpoint");
  }

  ntop->getTrace()->traceEvent(
      TRACE_NORMAL, "Successfully created ZMQ endpoint %s", endpoint);
}
