...
void HostChecksLoader::registerChecks() {
  ...
  if ((fcb = new CountriesContacts()))
    registerCheck(fcb);
  if ((fcb = new FlowFlood()))
    registerCheck(fcb);
  if ((fcb = new SYNScan()))
    registerCheck(fcb);
  if ((fcb = new SYNFlood()))
    registerCheck(fcb);
  if ((fcb = new DNSServerContacts()))
    registerCheck(fcb);
  if ((fcb = new SMTPServerContacts()))
    registerCheck(fcb);
  if ((fcb = new NTPServerContacts()))
    registerCheck(fcb);
  if ((fcb = new NTPTraffic()))
    registerCheck(fcb);
  if ((fcb = new P2PTraffic()))
    registerCheck(fcb);
  if ((fcb = new DNSTraffic()))
    registerCheck(fcb);
  if ((fcb = new RemoteConnection()))
    registerCheck(fcb);
  if ((fcb = new DangerousHost()))
    registerCheck(fcb);
  if ((fcb = new DomainNamesContacts()))
    registerCheck(fcb);
  if ((fcb = new ScoreThreshold()))
    registerCheck(fcb);
  if ((fcb = new ICMPFlood()))
    registerCheck(fcb);
  if ((fcb = new PktThreshold()))
    registerCheck(fcb);

#ifdef NTOPNG_PRO
  if ((fcb = new ScoreAnomaly()))
    registerCheck(fcb);
  if ((fcb = new FlowAnomaly()))
    registerCheck(fcb);
#endif
    ...
    for (std::map<std::string, u_int32_t>::const_iterator it = hooks.begin();
      ...
      if (json_object_object_get_ex(
        ...
        if (cb_all.find(check_key) != cb_all.end()) {
          ...
          ntop->getTrace()->traceEvent(
              TRACE_INFO, "Unable to find host check %s", check_key);
        }
      }
    }
