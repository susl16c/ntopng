...
FlowAlertsLoader::FlowAlertsLoader() {
  ...
  registerAlert(TLSMaliciousSignatureAlert::getClassType(),
                TLSMaliciousSignatureAlert::getDefaultScore());
  registerAlert(UDPUnidirectionalAlert::getClassType(),
                UDPUnidirectionalAlert::getDefaultScore());
...
ndpi_risk_enum
FlowAlertsLoader::getAlertRisk(FlowAlertTypeEnum alert_id) const {
  if (alert_id < MAX_DEFINED_FLOW_ALERT_TYPE)
