...
pfring *PF_RINGInterface::pfringSocketInit(const char *name) {
  ...
  flags |= PF_RING_DNA_SYMMETRIC_RSS; /* Note that symmetric RSS is ignored by
                                         non-DNA drivers */
  ...
  if (version != RING_VERSION_NUM) {
    ntop->getTrace()->traceEvent(
        TRACE_ERROR,
        "PF_RING version mismatch (expected v.%d.%d.%d found v.%d.%d.%d)",
        (RING_VERSION_NUM & 0xFFFF0000) >> 16,
        (RING_VERSION_NUM & 0x0000FF00) >> 8, RING_VERSION_NUM & 0x000000FF,
  ...
  ntop->getTrace()->traceEvent(
      TRACE_NORMAL, "Reading packets from PF_RING v.%d.%d.%d interface %s...",
      (version & 0xFFFF0000) >> 16, (version & 0x0000FF00) >> 8,
      version & 0x000000FF, name);
  ...
  if (pfring_set_socket_mode(handle, recv_only_mode) != 0)
    ntop->getTrace()->traceEvent(TRACE_WARNING,
                                 "Unable to set socket mode on %s", name);
...
void PF_RINGInterface::multiPacketPollLoop() {
  ...
  while (isRunning()) {
    rc = pfring_recv(pfring_handle[idx], &buffer, 0, &hdr,
                     0 /* wait_for_packet */);
...
void PF_RINGInterface::shutdown() {
  for (int i = 0; i < num_pfring_handles; i++) {
    if (pfring_handle[i])
      pfring_breakloop(pfring_handle[i]);
  }
...
u_int32_t PF_RINGInterface::getNumDroppedPackets() { return dropped_packets; }
...
bool PF_RINGInterface::set_packet_filter(char *filter) {
  ...
  ntop->getTrace()->traceEvent(TRACE_NORMAL,
                               "Packet capture filter set to \"%s\"", filter);
