...
#endif
{
  ...
  */
  ...
  if ((ntop = new (std::nothrow) Ntop(argv[0])) == NULL)
    _exit(0);
  if ((prefs = new (std::nothrow) Prefs(ntop)) == NULL)
    _exit(0);
    ...
    (!defined(__i686__)) && (!defined(__ARM_ARCH))
...
#endif
      {
        ...
        if (strncmp(ifName, "bridge:", 7) == 0) {
          ...
          ntop->getTrace()->traceEvent(
              TRACE_WARNING, "https://www.ntop.org/support/faq/"
                             "migration-of-ntopng-inline-pro-enterprises-"
                             "licenses-to-ntopng-edge-nedge/");
          ntop->getTrace()->traceEvent(TRACE_WARNING, "\n");
        }
#endif
  ...
  */
  ...
  /* This method returns after a shutdown has been requested.
     runHousekeepingTasks() is performed within this method untile it has
     returned. */
  ntop->start();
