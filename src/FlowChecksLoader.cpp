...
void FlowChecksLoader::registerCheck(FlowCheck *cb) {
  ...
  /*
    If this is a check that handles an nDPI flow risk, the corresponding risk is
    cleared in the unhandled risks bitmap
   */
  FlowRisk *fr = dynamic_cast<FlowRisk *>(cb);
  if (fr)
    NDPI_CLR_BIT(unhandled_ndpi_risks, fr->handledRisk());
}
/* **************************************************** */
...
void FlowChecksLoader::registerChecks() {
  ...
  if ((fcb = new BlacklistedFlow()))
    registerCheck(fcb);
  if ((fcb = new BlacklistedCountry()))
    registerCheck(fcb);
  if ((fcb = new BroadcastNonUDPTraffic()))
    registerCheck(fcb);
  if ((fcb = new DeviceProtocolNotAllowed()))
    registerCheck(fcb);
#ifndef NTOPNG_PRO
  if ((fcb = new ExternalAlertCheck()))
    registerCheck(fcb);
#endif
  if ((fcb = new FlowRiskBinaryApplicationTransfer()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskDNSSuspiciousTraffic()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskHTTPNumericIPHost()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskHTTPSuspiciousHeader()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskHTTPSuspiciousUserAgent()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskHTTPSuspiciousURL()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskKnownProtocolOnNonStandardPort()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskMalformedPacket()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskSMBInsecureVersion()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskSSHObsoleteServer()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskSSHObsoleteClient()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskSuspiciousDGADomain()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskTLSMissingSNI()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskTLSNotCarryingHTTPS()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskTLSSuspiciousESNIUsage()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskUnsafeProtocol()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskURLPossibleXSS()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskURLPossibleRCEInjection()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskURLPossibleSQLInjection()))
    registerCheck(fcb);
  if ((fcb = new IECUnexpectedTypeId()))
    registerCheck(fcb);
  if ((fcb = new IECInvalidTransition()))
    registerCheck(fcb);
  if ((fcb = new LowGoodputFlow()))
    registerCheck(fcb);
  if ((fcb = new NotPurged()))
    registerCheck(fcb);
  if ((fcb = new RemoteAccess()))
    registerCheck(fcb);
  if ((fcb = new RemoteToLocalInsecureProto()))
    registerCheck(fcb);
  if ((fcb = new RemoteToRemote()))
    registerCheck(fcb);
  if ((fcb = new TCPZeroWindow()))
    registerCheck(fcb);
  if ((fcb = new TCPNoDataExchanged()))
    registerCheck(fcb);
  if ((fcb = new UDPUnidirectional()))
    registerCheck(fcb);
  if ((fcb = new UnexpectedDNSServer()))
    registerCheck(fcb);
  if ((fcb = new UnexpectedDHCPServer()))
    registerCheck(fcb);
  if ((fcb = new UnexpectedNTPServer()))
    registerCheck(fcb);
  if ((fcb = new UnexpectedSMTPServer()))
    registerCheck(fcb);
  if ((fcb = new WebMining()))
    registerCheck(fcb);

#ifdef NTOPNG_PRO
  if ((fcb = new DataExfiltration()))
    registerCheck(fcb);
  if ((fcb = new DNSDataExfiltration()))
    registerCheck(fcb);
  if ((fcb = new ElephantFlow()))
    registerCheck(fcb);
  if ((fcb = new ExternalAlertCheckPro()))
    registerCheck(fcb);
  if ((fcb = new InvalidDNSQuery()))
    registerCheck(fcb);
#if !defined(HAVE_NEDGE)
  if ((fcb = new LateralMovement()))
    registerCheck(fcb);
  if ((fcb = new PeriodicityChanged()))
    registerCheck(fcb);
#endif
  if ((fcb = new LongLivedFlow()))
    registerCheck(fcb);
  if ((fcb = new TCPConnectionRefused()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskTLSCertValidityTooLong()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskTLSCertificateExpired()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskTLSCertificateMismatch()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskTLSOldProtocolVersion()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskTLSUnsafeCiphers()))
    registerCheck(fcb);
  if ((fcb = new FlowRiskTLSCertificateSelfSigned()))
    registerCheck(fcb);
  if ((fcb = new TLSMaliciousSignature()))
    registerCheck(fcb);
#ifdef HAVE_NEDGE
  if ((fcb = new NedgeBlockedFlow()))
    registerCheck(fcb);
#endif
#endif
  ...
  while (!json_object_iter_equal(&it, &itEnd)) {
    ...
    if (json_object_object_get_ex(check_config, "all", &json_hook_all)) {
      ...
      if (cb_all.find(check_key) != cb_all.end()) {
        ...
        ntop->getTrace()->traceEvent(TRACE_INFO, "Unable to find flow check %s",
                                     check_key);
    }
