...
HostStats::~HostStats() {
  ...
  if (quota_enforcement_stats)
    delete quota_enforcement_stats;
  ...
  if (tv->tv_sec >= periodicUpdate) {
    ...
    if (active_flows_cli.addObservation(host->getNumOutgoingFlows())) {
      ...
      ntop->getTrace()->traceEvent(
          ...
          host->get_ip()->print(buf, sizeof(buf)), iface->get_name(),
...
void HostStats::luaActiveFlowsBehaviour(lua_State *vm) {
  ...
  lua_push_uint64_table_entry(vm, "tot_num_anomalies",
                              active_flows_srv.getTotAnomalies() +
                                  active_flows_cli.getTotAnomalies());
...
void HostStats::luaScoreBehaviour(lua_State *vm) {
  ...
  lua_push_uint64_table_entry(vm, "tot_num_anomalies",
                              score_cli.getTotAnomalies() +
                                  score_srv.getTotAnomalies());
...
void HostStats::luaStats(lua_State *vm, NetworkInterface *iface,
  ...
  if (verbose) {
    if (ndpiStats)
      ndpiStats->lua(iface, vm, true, tsLua);
    if (dscpStats)
      dscpStats->lua(iface, vm, tsLua);
  }
...
void HostStats::lua(lua_State *vm, bool mask_host, DetailsLevel details_level) {
  ...
  lua_push_bool_table_entry(vm, "tcp.packets.seq_problems",
                            tcp_packet_stats_sent.seqIssues() ||
                                    tcp_packet_stats_rcvd.seqIssues()
                                ? true
                                : false);
  ...
  if (details_level >= details_high) {
    ...
    luaStats(vm, iface, details_level >= details_higher,
...
u_int32_t HostStats::getTotalAlerts() const { return total_alerts; }
...
void HostStats::incStats(time_t when, u_int8_t l4_proto, u_int ndpi_proto,
  ...
  if (custom_app.pen &&
      (custom_app_stats ||
       (custom_app_stats = new (std::nothrow) CustomAppStats(iface)))) {
