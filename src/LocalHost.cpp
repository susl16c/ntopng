...
LocalHost::LocalHost(NetworkInterface *_iface, Mac *_mac, VLANid _vlanId,
...
LocalHost::LocalHost(NetworkInterface *_iface, char *ipAddress, VLANid _vlanId,
...
void LocalHost::set_hash_entry_state_idle() {
  /* Serialization is performed, inline, as soon as the LocalHost becomes idle,
     and not when it is deleted. This guarantees that, if the same host becomes
     active again, its counters will be consistent even if its other instance
...
void LocalHost::initialize() {
  ...
  PROFILING_SUB_SECTION_ENTER(iface, "LocalHost::initialize: local_host_cache",
                              16);
