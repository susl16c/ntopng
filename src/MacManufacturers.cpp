...
void MacManufacturers::init() {
  ...
  if (!(stat(manufacturers_file, &buf) == 0) && (S_ISREG(buf.st_mode)))
    ntop->getTrace()->traceEvent(TRACE_ERROR,
                                 "File %s doesn't exists or is not readable",
                                 manufacturers_file);
        ...
        */
        ...
        if (mac_manufacturers.find(mac_key) == mac_manufacturers.end()) {
          ...
          ntop->getTrace()->traceEvent(
              TRACE_NORMAL, "Adding mac %02x:%02x:%02x [manufacturer name: %s]",
              s.mac_manufacturer[0], s.mac_manufacturer[1],
              s.mac_manufacturer[2], s.manufacturer_name);
#endif
        }
      }
    }
