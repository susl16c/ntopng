...
bool MySQLDB::connectToDB(MYSQL *conn, bool select_db) {
  ...
  if (mysql_init(conn) == NULL) {
    ntop->getTrace()->traceEvent(
        TRACE_ERROR, "Failed to initialize %s connection",
        ntop->getPrefs()->useClickHouse() ? "ClickHouse" : "MySQL");
  ...
  ntop->getTrace()->traceEvent(
      TRACE_NORMAL, "Successfully connected to %s [%s@%s:%i] for interface %s",
      ntop->getPrefs()->useClickHouse() ? "ClickHouse" : "MySQL", user, host,
      port, iface->get_name());
...
int MySQLDB::exec_single_query(lua_State *vm, char *sql) {
  ...
  if (enable_db_traces) {
    ...
    ntop->getTrace()->traceEvent(TRACE_NORMAL, "Query completed in %.3f sec",
                                 Utils::usecTimevalDiff(&end, &begin) /
                                     1000000.);
  }
