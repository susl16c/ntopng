...
#ifdef WIN32
  if (name == NULL)
    name = "1"; /* First available interface */
#endif
...
void NetworkInterface::init() {
  ...
  customIftype = NULL, is_loopback = is_traffic_mirrored = false,
  lbd_serialize_by_mac = false, discard_probing_traffic = false;
  ...
  running = false, interfaceStats = NULL,
  has_too_many_hosts = has_too_many_flows = false, flow_dump_disabled = false,
...
struct ndpi_detection_module_struct *NetworkInterface::initnDPIStruct() {
  ...
  ndpi_set_proto_defaults(ndpi_s, 0, NDPI_PROTOCOL_UNRATED,
...
void NetworkInterface::nDPILoadIPCategory(char *what,
...
void NetworkInterface::nDPILoadHostnameCategory(char *what,
                                                ndpi_protocol_category_t id) {
  // ntop->getTrace()->traceEvent(TRACE_NORMAL, "%s(%p) [%s]", __FUNCTION__,
  // ndpi_struct_shadow, what);
...
NetworkInterface::get_ndpi_proto_category(u_int protoid) {
  ndpi_protocol proto;
...
NetworkInterface::~NetworkInterface() {
    ...
    for (u_int i = 0; i < PROFILING_NUM_SECTIONS; i++) {
      if (PROFILING_SECTION_LABEL(i) != NULL)
        ntop->getTrace()->traceEvent(
            ...
            PROFILING_SECTION_LABEL(i), PROFILING_SECTION_AVG(i, n));
    }
  }
#endif
  ...
  if (host_pools)
    delete host_pools; /* note: this requires ndpi_struct */
  if (bcast_domains)
    delete bcast_domains;
  if (ifDescription)
    free(ifDescription);
  if (discovery)
    delete discovery;
...
#ifndef HAVE_NEDGE
  if (flow_profiles)
    delete (flow_profiles);
  if (shadow_flow_profiles)
    delete (shadow_flow_profiles);
  if (sub_interfaces)
    delete (sub_interfaces);
#endif
  if (custom_app_stats)
    delete custom_app_stats;
  if (flow_interfaces_stats)
    delete flow_interfaces_stats;
#endif
  if (hide_from_top)
    delete (hide_from_top);
  if (hide_from_top_shadow)
    delete (hide_from_top_shadow);
  if (influxdb_ts_exporter)
    delete influxdb_ts_exporter;
  if (rrd_ts_exporter)
    delete rrd_ts_exporter;
  if (dhcp_ranges)
    delete[] dhcp_ranges;
  ...
  if (flowAlertsQueue)
    delete flowAlertsQueue;
  if (hostAlertsQueue)
    delete hostAlertsQueue;
  ...
  if (prev_flow_checks_executor)
    delete prev_flow_checks_executor;
  if (flow_checks_executor)
    delete flow_checks_executor;

  if (prev_host_checks_executor)
    delete prev_host_checks_executor;
  if (host_checks_executor)
    delete host_checks_executor;
...
bool NetworkInterface::walker(u_int32_t *begin_slot, bool walk_all,
  ...
  switch (wtype) {
  case walker_hosts:
    ret = hosts_hash ? hosts_hash->walk(begin_slot, walk_all, walker, user_data)
                     : false;
    break;

  case walker_flows:
    ret = flows_hash ? flows_hash->walk(begin_slot, walk_all, walker, user_data)
                     : false;
    break;
  ...
  case walker_vlans:
    ret = vlans_hash ? vlans_hash->walk(begin_slot, walk_all, walker, user_data)
                     : false;
    break;
  }
...
Flow *NetworkInterface::getFlow(
    Mac *srcMac, Mac *dstMac, VLANid vlan_id, u_int16_t observation_domain_id,
  ...
  ret = flows_hash->find(src_ip, dst_ip, src_port, dst_port, vlan_id,
                         observation_domain_id, l4_proto, icmp_info,
                         src2dst_direction, true /* Inline call */);
  ...
  if (ret == NULL) {
    ...
    *new_flow = true;
    ...
    try {
      ...
      ret = new (std::nothrow)
          Flow(this, vlan_id, observation_domain_id, l4_proto, srcMac, src_ip,
               src_port, dstMac, dst_ip, dst_port, icmp_info, first_seen,
  ...
  if (srcMac) {
    if ((srcHost = (*src2dst_direction) ? ret->get_cli_host()
                                        : ret->get_srv_host())) {
      if ((!srcMac->isSpecialMac()) && (primary_mac = srcHost->getMac()) &&
          primary_mac != srcMac) {
#ifdef MAC_DEBUG
  ...
  if (dstMac) {
    if ((dstHost = (*src2dst_direction) ? ret->get_srv_host()
                                        : ret->get_cli_host())) {
      if ((!dstMac->isSpecialMac()) && (primary_mac = dstHost->getMac()) &&
          primary_mac != dstMac) {
#ifdef MAC_DEBUG
        char buf[32], bufm1[32], bufm2[32];
        ntop->getTrace()->traceEvent(
...
void NetworkInterface::setSubInterface(FlowHashingEnum mode,
                                       u_int64_t criteria) {
  is_dynamic_interface = true, dynamic_interface_mode = mode,
...
bool NetworkInterface::processPacket(
    ...
    struct ndpi_ethhdr *eth, VLANid vlan_id, struct ndpi_iphdr *iph,
...
pre_get_flow:
  /* Updating Flow */
  flow = getFlow(
      srcMac, dstMac, vlan_id, 0 /* observationPointId */, 0, 0, 0,
      l4_proto == IPPROTO_ICMP ? &icmp_info : NULL, &src_ip, &dst_ip, src_port,
      dst_port, l4_proto, &src2dst_direction, last_pkt_rcvd, last_pkt_rcvd,
      len_on_wire, &new_flow, true, eth->h_source,
  ...
  if (flow == NULL) {
    ...
    if (isTrafficMirrored() && (!isGwMacConfigured())) {
      ...
      ntop->getTrace()->traceEvent(TRACE_NORMAL, "%s -> %s",
                                   flow->get_cli_ip_addr()->isLocalHost(&network_id)
      ? "L" : "R", flow->get_srv_ip_addr()->isLocalHost(&network_id) ? "L" :
      "R");
      */
    }
    ...
    *hostFlow = flow;
    ...
    case IPPROTO_TCP:
      ...
      /*
        ...
        The only condition that should NOT be checked for swap is when there's a
        SYN and not an ACK, i.e., when we see the first packet of the TWH that
        allows to reliably determine the direction.
       */
...
datalink_check:
  if (pcap_datalink_type == DLT_NULL) {
    ...
    if (sender_mac)
      memcpy(&dummy_ethernet.h_source, sender_mac, 6);
...
decode_packet_eth:
  ...
  case ETHERTYPE_PPPoE:
    ...
    default:
      incStats(ingressPacket, h->ts.tv_sec, ETHERTYPE_IP, NDPI_PROTOCOL_UNKNOWN,
               NDPI_PROTOCOL_CATEGORY_UNSPECIFIED, 0, len_on_wire, 1);
      goto dissect_packet_end;
    }
  ...
  case ETHERTYPE_IP:
    if (h->caplen >= ip_offset + sizeof(struct ndpi_iphdr)) {
      ...
      if (ntop->getPrefs()->do_ignore_macs())
        ...
        dummy_ethernet.h_dest[4] = 0x8e, dummy_ethernet.h_dest[5] = rand() % 8;
        ethernet = &dummy_ethernet;
      }
      ...
      try {
        pass_verdict = processPacket(
            ...
            iph, ip6, ip_offset, encapsulation_overhead, len_on_wire, h, packet,
            ndpiProtocol, srcHost, dstHost, flow);
      } catch (std::bad_alloc &ba) {
        static bool oom_warning_sent = false;
  ...
  default: /* No IPv4 nor IPv6 */
...
#ifndef HAVE_NEDGE
    if (srcMac)
      srcMac->incSentStats(h->ts.tv_sec, 1, len_on_wire);
    if (dstMac)
      dstMac->incRcvdStats(h->ts.tv_sec, 1, len_on_wire);
#endif
    ...
    if ((eth_type == ETHERTYPE_ARP) &&
      ...
      /* Updates interface broadcast domains, according to what is seen in this
       * ARP that glues together L2 and L3 */
...
void NetworkInterface::pollQueuedeCompanionEvents() {
  if (companionQueue) {
    ...
    while (dequeueFlowFromCompanion(&dequeued)) {
      ...
      flow =
          getFlow(NULL /* srcMac */, NULL /* dstMac */, 0 /* vlan_id */,
                  0 /* observationPointId */, 0 /* deviceIP */, 0 /* inIndex */,
                  1 /* outIndex */, NULL /* ICMPinfo */, &dequeued->src_ip,
                  &dequeued->dst_ip, dequeued->src_port, dequeued->dst_port,
                  dequeued->l4_proto, &src2dst_direction, 0, 0, 0, &new_flow,
...
u_int64_t NetworkInterface::dequeueFlowAlertsFromChecks(u_int budget) {
  ...
  if (num_done == 0) {
    /*
      ...
      as we must ensure purgeQueuedIdleFlows() gets executed, and also to exit
      when it's time to shutdown.
    */
    struct timespec hooks_wait_expire;

    hooks_wait_expire.tv_sec = time(NULL) + 1, hooks_wait_expire.tv_nsec = 0;
...
u_int64_t NetworkInterface::dequeueHostAlertsFromChecks(u_int budget) {
  ...
  if (num_done == 0) {
    /*
      ...
      as we must ensure purgeQueuedIdleHosts() gets executed, and also to exit
      when it's time to shutdown.
    */
    struct timespec hooks_wait_expire;
...
u_int64_t NetworkInterface::dequeueFlowsForDump(u_int idle_flows_budget,
  ...
  */
  ...
  if (!dumper) {
    ntop->getTrace()->traceEvent(TRACE_INFO,
                                 "WARNING: Something is broken with flow dump");
    return (0);
  }
  ...
  while (idleFlowsToDump->isNotEmpty()) {
    ...
    if (f->get_partial_bytes()) /* Make sure data is not at zero */
      rc = dumper->dumpFlow(f->get_last_seen(), f,
                            json); /* Finally dump this flow */
  ...
  */
  ...
  if (!isViewed() && num_done == 0) {
    ...
    dump_wait_expire.tv_sec = time(NULL) + 1, dump_wait_expire.tv_nsec = 0;
...
void NetworkInterface::hostAlertsDequeueLoop() {
  ...
  ntop->getTrace()->traceEvent(TRACE_NORMAL,
                               "Host dump thread completed for %s", get_name());
}
...
void NetworkInterface::dumpFlowLoop() {
  ...
  ntop->getTrace()->traceEvent(TRACE_NORMAL, "Flow dump loop completed for %s",
                               get_name());
}
...
void NetworkInterface::shutdown() {
  ...
  if (running) {
    ...
    if (flowDumpLoopCreated)
      pthread_join(flowDumpLoop, &res);
    if (flowAlertsDequeueLoopCreated)
      pthread_join(flowChecksLoop, &res);
    if (hostAlertsDequeueLoopCreated)
      pthread_join(hostChecksLoop, &res);
...
void NetworkInterface::findFlowHosts(VLANid vlanId,
                                     ...
                                     Mac *src_mac, IpAddress *_src_ip,
                                     Host **src, Mac *dst_mac,
  ...
  PROFILING_SECTION_ENTER("NetworkInterface::findFlowHosts: hosts_hash->get",
                          3);
  ...
  if ((*src) == NULL) {
    ...
    if (_src_ip && (_src_ip->isLocalHost(&local_network_id) ||
                    _src_ip->isLocalInterfaceAddress())) {
      PROFILING_SECTION_ENTER("NetworkInterface::findFlowHosts: new LocalHost",
                              4);
      (*src) = new (std::nothrow)
          LocalHost(this, src_mac, vlanId, observation_domain_id, _src_ip);
      PROFILING_SECTION_EXIT(4);
    } else {
      PROFILING_SECTION_ENTER("NetworkInterface::findFlowHosts: new RemoteHost",
                              5);
      (*src) = new (std::nothrow)
          RemoteHost(this, src_mac, vlanId, observation_domain_id, _src_ip);
  ...
  if ((*dst) == NULL) {
    ...
    if (_dst_ip && (_dst_ip->isLocalHost(&local_network_id) ||
                    _dst_ip->isLocalInterfaceAddress())) {
      PROFILING_SECTION_ENTER("NetworkInterface::findFlowHosts: new LocalHost",
                              4);
      (*dst) = new (std::nothrow)
          LocalHost(this, dst_mac, vlanId, observation_domain_id, _dst_ip);
      PROFILING_SECTION_EXIT(4);
    } else {
      PROFILING_SECTION_ENTER("NetworkInterface::findFlowHosts: new RemoteHost",
                              5);
      (*dst) = new (std::nothrow)
          RemoteHost(this, dst_mac, vlanId, observation_domain_id, _dst_ip);
      PROFILING_SECTION_EXIT(5);
    }
...
void NetworkInterface::periodicStatsUpdate() {
...
#ifdef NTOPNG_PRO
  if (getHostPools())
    getHostPools()->checkPoolsStatsReset();
#endif
  ...
  for (u_int8_t network_id = 0; network_id < ntop->getNumLocalNetworks();
       network_id++) {
  ...
  if (tv.tv_sec >= nextMinPeriodicUpdate) {
    ...
    if (sMap)
      sMap->purgeIdle(nextMinPeriodicUpdate);
    if (pMap)
      pMap->purgeIdle(nextMinPeriodicUpdate);
...
Host *NetworkInterface::getHost(char *host_ip, VLANid vlan_id,
  ...
  if ((inet_pton(AF_INET, (const char *)host_ip, &a4) == 0) &&
    ...
    if (ip) {
      ...
      h = hosts_hash
              ? hosts_hash->get(vlan_id, ip, isInlineCall, observation_point_id)
              : NULL;
...
Host *NetworkInterface::findHostByIP(AddressTree *allowed_hosts, char *host_ip,
                                     ...
                                     u_int16_t observationPointId) {
  if (host_ip != NULL) {
    Host *h = getHost(host_ip, vlan_id, observationPointId,
                      false /* Not an inline call */);
...
static bool flow_matches(Flow *f, struct flowHostRetriever *retriever) {
  ...
  TcpFlowStateFilter tcp_flow_state_filter;
  bool unicast, unidirectional, alerted_flows, cli_pool_found = false,
                                               srv_pool_found = false;
  ...
  if (f && (!f->idle())) {
    ...
    if (retriever->pag && retriever->pag->l7protoFilter(&ndpi_proto) &&
    ...
    if (retriever->pag && retriever->pag->clientMode(&client_policy) &&
        (((client_policy == location_local_only) &&
          (!f->get_cli_ip_addr()->isLocalHost(&local_network_id))) ||
    ...
    if (retriever->pag && retriever->pag->serverMode(&server_policy) &&
        (((server_policy == location_local_only) &&
          (!f->get_srv_ip_addr()->isLocalHost(&local_network_id))) ||
    ...
    if (retriever->pag &&
      ...
      if (!f->isFlowAlerted() ||
          ...
          (flow_status_severity_filter == alert_level_group_error_or_higher &&
...
void NetworkInterface::getActiveFlowsStats(nDPIStats *ndpi_stats,
                                           FlowStats *stats,
...
int NetworkInterface::getFlows(lua_State *vm, u_int32_t *begin_slot,
  ...
  if (sortFlows(begin_slot, walk_all, &retriever, allowed_hosts, host, p,
                sortColumn) < 0) {
    return (-1);
  }
...
int NetworkInterface::getFlowsGroup(lua_State *vm, AddressTree *allowed_hosts,
  ...
  if (sortFlows(&begin_slot, walk_all, &retriever, allowed_hosts, NULL, p,
                groupColumn) < 0) {
    return (-1);
  }
...
int NetworkInterface::sortHosts(
    ...
    VLANid vlan_id, OSType osFilter, u_int32_t asnFilter, int16_t networkFilter,
    u_int16_t pool_filter, bool filtered_hosts, bool blacklisted_hosts,
    bool hide_top_hidden, bool anomalousOnly, bool dhcpOnly,
  ...
  if ((!strcmp(sortColumn, "column_ip")) || (!strcmp(sortColumn, "column_")))
    retriever->sorter = column_ip, sorter = hostSorter;
  else if (!strcmp(sortColumn, "column_vlan"))
    retriever->sorter = column_vlan, sorter = numericSorter;
...
int NetworkInterface::getActiveHostsList(
    ...
    VLANid vlan_id, OSType osFilter, u_int32_t asnFilter, int16_t networkFilter,
    u_int16_t pool_filter, bool filtered_hosts, bool blacklisted_hosts,
    bool hide_top_hidden, u_int8_t ipver_filter, int proto_filter,
  ...
  if (sortHosts(begin_slot, walk_all, &retriever, bridge_iface_idx,
                ...
                pool_filter, filtered_hosts, blacklisted_hosts, hide_top_hidden,
                anomalousOnly, dhcpOnly, cidr_filter, ipver_filter,
                proto_filter, traffic_type_filter, device_ip, sortColumn) < 0) {
    return (-1);
  }
...
u_int NetworkInterface::getNumL2Devices() { return (numL2Devices); };

/* **************************************************** */

u_int NetworkInterface::getNumHosts() { return (numHosts); };

/* **************************************************** */

u_int NetworkInterface::getNumLocalHosts() { return (numLocalHosts); };
...
void NetworkInterface::lua(lua_State *vm) {
  ...
  lua_push_bool_table_entry(
      vm, "isDynamic",
      isSubInterface()); /* An runtime-instantiated interface */
  lua_push_bool_table_entry(
      vm, "isSampledTraffic",
...
void NetworkInterface::lua_queues_stats(lua_State *vm) {
  if (idleFlowsToDump)
    idleFlowsToDump->lua(vm);
  if (activeFlowsToDump)
    activeFlowsToDump->lua(vm);
  if (flowAlertsQueue)
    flowAlertsQueue->lua(vm);
}
...
VLAN *NetworkInterface::getVLAN(VLANid vlanId, bool create_if_not_present,
                                bool isInlineCall) {
  VLAN *ret = NULL;
...
Flow *NetworkInterface::findFlowByTuple(VLANid vlan_id,
  ...
  f = (Flow *)flows_hash->find(src_ip, dst_ip, src_port, dst_port, vlan_id,
                               observation_domain_id, l4_proto, NULL, &src2dst,
                               false /* Not an inline call */);
...
static bool host_reload_hide_from_top(GenericHashEntry *host, void *user_data,
                                      bool *matched) {
  Host *h = (Host *)host;
...
void NetworkInterface::reloadHideFromTop(bool refreshHosts) {
  ...
  if (refreshHosts) {
    ...
    u_int32_t begin_slot = 0;
    bool walk_all = true;
    walker(&begin_slot, walk_all, walker_hosts, host_reload_hide_from_top,
           NULL);
  }
}
...
bool NetworkInterface::isHiddenFromTop(Host *host) {
  ...
  return (host->get_ip()->findAddress(
      vlan_addrtree->getAddressTree(host->get_vlan_id())));
}
...
bool NetworkInterface::initFlowChecksLoop() {
  if (isView()) /* Don't init the loop for view interfaces: the loop is run by
                   every viewed interface independently */
    return true;
  ...
  for (u_int i = 0, num_found = 0;
    ...
    if (live_captures[i] != NULL) {
      ...
      /* The header is always sent even when there is never a match with
         matchLiveCapture, as otherwise some browsers may end up in hangning.
         Hanging has been verified with Safari Version 12.0 (13606.2.11)
      ...
      if (!http_client_disconnected && c->conn &&
        ...
        if (((res = mg_write_async(c->conn, &pkthdr, sizeof(pkthdr))) <
             (int)sizeof(pkthdr)) ||
            ((res = mg_write_async(c->conn, packet, h->caplen)) <
             (int)h->caplen))
          http_client_disconnected = true;
        else {
...
void NetworkInterface::walkAlertables(AlertEntity alert_entity,
  ...
  if (((alert_entity == alert_entity_none) ||
    ...
    for (u_int8_t network_id = 0; network_id < num_local_networks;
         network_id++) {
...
u_int32_t
NetworkInterface::getNumEngagedAlerts(AlertLevelGroup alert_level_group) const {
  u_int32_t tot_engaged_alerts = 0;
...
void NetworkInterface::luaNumEngagedAlerts(lua_State *vm) const {
  ...
  lua_push_uint64_table_entry(
      vm, "error", getNumEngagedAlerts(alert_level_group_error_or_higher));
...
void NetworkInterface::processExternalAlertable(
    AlertEntity entity, const char *entity_val, bool create_if_missing,
    lua_State *vm, u_int vm_argument_idx, bool do_store_alert) {
  std::map<std::pair<AlertEntity, std::string>,
           InterfaceMemberAlertableEntity *>::iterator it;
  ...
  /* Create */
  if ((alertable = new (std::nothrow)
           InterfaceMemberAlertableEntity(this, entity)) == NULL) {
    external_alerts_lock.unlock(__FILE__, __LINE__);
    ntop->getTrace()->traceEvent(TRACE_ERROR, "Not enough memory");
    lua_pushnil(vm);
    return;
...
void NetworkInterface::luaAlertedFlows(lua_State *vm) {
  ...
  lua_push_int32_table_entry(
      vm, "num_alerted_flows_error",
      getNumActiveAlertedFlows(alert_level_group_error_or_higher));
}
...
void NetworkInterface::luaPeriodicityMap(
    lua_State *vm, const u_int8_t *const mac, IpAddress *ip_address,
    VLANid vlan_id, u_int16_t host_pool_id, bool unicast, u_int32_t first_seen,
    u_int16_t filter_ndpi_proto, u_int32_t maxHits) {
#if defined(NTOPNG_PRO) && !defined(HAVE_NEDGE)
  if (pMap) {
    pMap->lua(vm, true, this, mac, ip_address, vlan_id, host_pool_id, unicast,
              0, filter_ndpi_proto, first_seen, maxHits);
    return;
  }
#endif
...
void NetworkInterface::luaServiceMap(lua_State *vm, const u_int8_t *const mac,
                                     IpAddress *ip_address, VLANid vlan_id,
                                     u_int16_t host_pool_id, bool unicast,
                                     u_int32_t first_seen,
                                     u_int16_t filter_ndpi_proto,
                                     u_int32_t maxHits) {
#if defined(NTOPNG_PRO)
  if (sMap) {
    sMap->lua(vm, false, this, mac, ip_address, vlan_id, host_pool_id, unicast,
              0, filter_ndpi_proto, first_seen, maxHits);
    return;
  }
#endif
...
void NetworkInterface::luaAnomalies(lua_State *vm) {
  /* Anomalies */
  lua_newtable(vm);
  if (has_too_many_flows)
    lua_push_bool_table_entry(vm, "too_many_flows", true);
  if (has_too_many_hosts)
    lua_push_bool_table_entry(vm, "too_many_hosts", true);
