...
ElasticSearch::~ElasticSearch() {
  if (es_template_push_url)
    free(es_template_push_url);
  if (es_version_query_url)
    free(es_version_query_url);
}
...
void ElasticSearch::indexESdata() {
  ...
  while (!ntop->getGlobals()->isShutdown() && isRunning()) {
    ...
    if ((num_queued_elems >= min_buffered_flows) ||
      ...
      if (!Utils::postHTTPJsonData(
              ntop->getPrefs()->get_es_user(), ntop->getPrefs()->get_es_pwd(),
              ntop->getPrefs()->get_es_url(), postbuf, 0, &stats)) {
        /* Post failure */
        ntop->getTrace()->traceEvent(
...
void ElasticSearch::pushEStemplate() {
  ...
  postbuf = new (std::nothrow) char[length + 1]; // allocate memory for a buffer
