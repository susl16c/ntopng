...
Prefs::Prefs(Ntop *_ntop) {
  ...
  enable_flow_device_port_rrd_creation =
      enable_observation_points_rrd_creation = false;
  ...
  dump_flows_on_es = dump_flows_on_mysql = dump_flows_on_syslog =
      dump_flows_on_nindex = false;
  dump_json_flows_on_disk = load_json_flows_from_disk_to_nindex =
      dump_ext_json = false;
...
Prefs::~Prefs() {
  if (ifNames) {
    for (int i = 0; i < num_interfaces; i++) {
      if (ifNames[i].name)
        free(ifNames[i].name);
      if (ifNames[i].description)
        free(ifNames[i].description);
    }
  ...
  if (zmq_publish_events_url)
    free(zmq_publish_events_url);
  if (data_dir)
    free(data_dir);
  if (install_dir)
    free(install_dir);
  ...
  if (cpu_affinity)
    free(cpu_affinity);
  if (other_cpu_affinity)
    free(other_cpu_affinity);
  if (es_type)
    free(es_type);
  if (es_index)
    free(es_index);
  ...
  if (ls_proto)
    free(ls_proto);
  if (http_binding_address1)
    free(http_binding_address1);
  if (http_binding_address2)
    free(http_binding_address2);
  if (https_binding_address1)
    free(https_binding_address1);
  if (https_binding_address2)
    free(https_binding_address2);
  if (lan_interface)
    free(lan_interface);
  if (wan_interface)
    free(wan_interface);
  if (ndpi_proto_path)
    free(ndpi_proto_path);
  if (test_pre_script_path)
    free(test_pre_script_path);
  if (test_post_script_path)
    free(test_post_script_path);
}
...
void usage() {
  printf(
      ...
      "resolve all\n"
      "                                    |     numeric IPs\n"
      "                                    | 2 - Decode DNS responses and "
      "don't\n"
      "                                    |     resolve numeric IPs\n"
      "                                    | 3 - Don't decode DNS responses "
      "and don't\n"
      "                                    |     resolve numeric IPs\n"
#endif
      ...
      "                                    | <local nets> is a comma-separated "
      "list of networks\n"
      "                                    | in CIDR format or a path to a "
      "file.\n"
      "                                    | The file accepts multiple lines "
      "with networks in CIDR format.\n"
      ...
      "strftime() format.\n"
      "                                    |   <mapping type>s have been "
      "removed starting at\n"
      "                                    |   ElasticSearch version 6. "
      "<mapping type> values whill therefore be\n"
      "                                    |   ignored when using versions "
...
void Prefs::reloadPrefsFromRedis() {
  ...
  enable_idle_local_hosts_cache =
      getDefaultBoolPrefsValue(CONST_RUNTIME_IDLE_LOCAL_HOSTS_CACHE_ENABLED,
...
void Prefs::refreshBehaviourAnalysis() {
  enable_behaviour_analysis =
      getDefaultBoolPrefsValue(CONST_PREFS_BEHAVIOUR_ANALYSIS, false);
...
int Prefs::setOption(int optkey, char *optarg) {
  ...
  case 'S':
    ntop->getTrace()->traceEvent(
        TRACE_ERROR, "-S deprecated, sticky hosts no longer supported. "
    ...
    if (optarg[0] != '/') {
      ...
      if (len > 0) {
        if (optarg[len - 1] == '/') {
          ntop->getTrace()->traceEvent(
              TRACE_WARNING,
              "-Z argument (%s) cannot end with '/' (example /ntopng): skipped",
              optarg);
        } else {
          free(http_prefix);
  ...
  case 'l':
    ...
    default:
      ntop->getTrace()->traceEvent(
          TRACE_ERROR, "Invalid '%s' value specified for -l: ignored", optarg);
    }
    break;
  ...
  case 'v': {
    if (!optarg)
      ntop->getTrace()->traceEvent(TRACE_ERROR,
                                   "No value specified for verbosity: ignored");
    else {
  ...
  case 'F':
    ...
    if (!optarg)
      ...
      if (elastic_index_type && elastic_index_name && elastic_url) {
        ...
        ntop->getTrace()->traceEvent(
            TRACE_NORMAL, "Using ElasticSearch for data dump [%s][%s][%s][%s]",
      ...
      if ((!sep) && (optarg[0] != 'c')) {
        ...
        if (use_clickhouse) {
          ...
          if (!client_found) {
            ntop->getTrace()->traceEvent(
                TRACE_WARNING,
                "-F clickhouse is not available (ClickHouse client not found)");
        ...
        if (all_good) {
          ...
          /*
             Old Format: mysql;<host[@port]|unix socket>;<dbname>;<table
             name>;<user>;<pw> New Format: mysql;<host[@[port,]port]|unix
             socket>;<dbname>;<user>;<pw>
          ...
          */
          ...
          if ((num_semicolumns == 0) && use_clickhouse) {
            ...
            if (num_semicolumns == 5) {
              ...
              optarg =
                  Utils::tokenizer(optarg, ';', &mysql_tablename); /* Skip it */
              if (mysql_tablename)
                free(mysql_tablename);
            }
          ...
          if (mysql_host && mysql_user) {
            ...
            ntop->getTrace()->traceEvent(
                ...
                use_clickhouse ? "clickhouse" : "mysql");
        } /* all_good */
      }
#else
      ntop->getTrace()->traceEvent(
  ...
  case 211:
    ntop->getTrace()->traceEvent(
        TRACE_WARNING, "Online check no longer supported (please remove "
  ...
  default:
    ntop->getTrace()->traceEvent(TRACE_WARNING, "Unknown option -%c: Ignored.",
                                 (char)optkey);
    return (-1);
  }
...
int Prefs::checkOptions() {
  ...
  if (print_version) {
    ...
    printf("%s\n", getLicenseJSON(
                       (char *)PACKAGE_VERSION, (char *)PACKAGE_OS, edition,
...
int Prefs::loadFromFile(const char *path) {
  ...
  while (true) {
    ...
    if (!strncmp(line, "--", 2)) { /* long opt */
      ...
      ntop->getTrace()->traceEvent(TRACE_WARNING,
                                   "Skipping unrecognized line: %s", line);
      continue;
    }
  }
...
void Prefs::lua(lua_State *vm) {
  ...
  lua_push_bool_table_entry(vm, "is_dump_flows_to_mysql_enabled",
                            dump_flows_on_mysql);
  if (mysql_dbname)
    lua_push_str_table_entry(vm, "mysql_dbname", mysql_dbname);
  ...
  lua_push_str_table_entry(vm, "other_cpu_affinity",
                           other_cpu_affinity ? other_cpu_affinity
                                              : (char *)"");
