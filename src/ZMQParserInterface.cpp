...
ZMQParserInterface::~ZMQParserInterface() {
  ...
  if (zmq_remote_stats)
    free(zmq_remote_stats);
  if (zmq_remote_stats_shadow)
    free(zmq_remote_stats_shadow);
#ifdef NTOPNG_PRO
  if (custom_app_maps)
    delete (custom_app_maps);
#endif
...
bool ZMQParserInterface::parsePENZeroField(ParsedFlow *const flow,
  ...
  case IPV6_DST_ADDR:
    if (flow->dst_ip.isEmpty()) {
      ...
      if (!ip_aux.isEmpty() &&
        ...
        ntop->getTrace()->traceEvent(
            TRACE_WARNING, "Attempt to set destination ip multiple times. "
                           "Check exported fields");
    }
    break;
  case L4_SRC_PORT:
  ...
  case BGP_PREV_ADJACENT_ASN:
    ...
    break;
  default:
    ntop->getTrace()->traceEvent(TRACE_INFO, "Skipping no-PEN flow fieldId %u",
                                 field);
    return false;
  }
...
bool ZMQParserInterface::parsePENNtopField(ParsedFlow *const flow,
  ...
  case HTTP_USER_AGENT:
    if (value->string && value->string[0] && value->string[0] != '\n') {
      if (flow->http_user_agent)
        free(flow->http_user_agent);
  ...
  case SSL_SERVER_NAME:
    if (value->string && value->string[0] && value->string[0] != '\n') {
      if (flow->tls_server_name)
        free(flow->tls_server_name);
  ...
  case BITTORRENT_HASH:
    if (value->string && value->string[0] && value->string[0] != '\n') {
      if (flow->bittorrent_hash)
        free(flow->bittorrent_hash);
...
bool ZMQParserInterface::parseNProbeAgentField(
  ...
  if (!strncmp(key, "timestamp", 9)) {
    ...
    if (json_object_object_get_ex(jvalue, "PID", &obj))
      flow->process_info.father_pid = (u_int32_t)json_object_get_int64(obj);
    if (json_object_object_get_ex(jvalue, "UID", &obj))
      flow->process_info.father_uid = (u_int32_t)json_object_get_int64(obj);
    if (json_object_object_get_ex(jvalue, "UID_NAME", &obj))
      flow->process_info.father_uid_name = (char *)json_object_get_string(obj);
    if (json_object_object_get_ex(jvalue, "GID", &obj))
      flow->process_info.father_gid = (u_int32_t)json_object_get_int64(obj);
    if (json_object_object_get_ex(jvalue, "VM_SIZE", &obj))
      flow->process_info.actual_memory = (u_int32_t)json_object_get_int64(obj);
    if (json_object_object_get_ex(jvalue, "VM_PEAK", &obj))
      flow->process_info.peak_memory = (u_int32_t)json_object_get_int64(obj);
    ...
    if (!flow->process_info_set)
      flow->process_info_set = true;
    ret = true;

    // ntop->getTrace()->traceEvent(TRACE_NORMAL, "Father Process [pid: %u][uid:
    // %u][gid: %u][path: %s]",
    //					 flow->process_info.father_pid,
    //flow->process_info.father_uid, 				 flow->process_info.father_gid,
    //				 flow->process_info.father_process_name);
  } else if (strlen(key) >= 7 &&
             !strncmp(&key[strlen(key) - 7], "PROCESS", 7)) {
    if (json_object_object_get_ex(jvalue, "PID", &obj))
      flow->process_info.pid = (u_int32_t)json_object_get_int64(obj);
    if (json_object_object_get_ex(jvalue, "UID", &obj))
      flow->process_info.uid = (u_int32_t)json_object_get_int64(obj);
    if (json_object_object_get_ex(jvalue, "UID_NAME", &obj))
      flow->process_info.uid_name = (char *)json_object_get_string(obj);
    if (json_object_object_get_ex(jvalue, "GID", &obj))
      flow->process_info.gid = (u_int32_t)json_object_get_int64(obj);
    if (json_object_object_get_ex(jvalue, "VM_SIZE", &obj))
      flow->process_info.actual_memory = (u_int32_t)json_object_get_int64(obj);
    if (json_object_object_get_ex(jvalue, "VM_PEAK", &obj))
      flow->process_info.peak_memory = (u_int32_t)json_object_get_int64(obj);
    if (json_object_object_get_ex(jvalue, "PROCESS_PATH", &obj))
      flow->process_info.process_name = (char *)json_object_get_string(obj);
    if (!flow->process_info_set)
      flow->process_info_set = true;
    ret = true;
    ...
    //				 flow->process_info.pid, flow->process_info.uid,
    //flow->process_info.gid, 				 flow->process_info.actual_memory,
    //flow->process_info.peak_memory, 				 flow->process_info.process_name);
    ...
    if (json_object_object_get_ex(jvalue, "CONN_STATE", &obj))
      flow->tcp_info.conn_state =
          Utils::tcpStateStr2State(json_object_get_string(obj));

    if (json_object_object_get_ex(jvalue, "SEGS_IN", &obj))
      flow->tcp_info.in_segs = (u_int32_t)json_object_get_int64(obj);
    if (json_object_object_get_ex(jvalue, "SEGS_OUT", &obj))
      flow->tcp_info.out_segs = (u_int32_t)json_object_get_int64(obj);
    if (json_object_object_get_ex(jvalue, "UNACK_SEGMENTS", &obj))
      flow->tcp_info.unacked_segs = (u_int32_t)json_object_get_int64(obj);
    if (json_object_object_get_ex(jvalue, "RETRAN_PKTS", &obj))
      flow->tcp_info.retx_pkts = (u_int32_t)json_object_get_int64(obj);
    if (json_object_object_get_ex(jvalue, "LOST_PKTS", &obj))
      flow->tcp_info.lost_pkts = (u_int32_t)json_object_get_int64(obj);

    if (json_object_object_get_ex(jvalue, "RTT", &obj))
      flow->tcp_info.rtt = json_object_get_double(obj);
    if (json_object_object_get_ex(jvalue, "RTT_VARIANCE", &obj))
      flow->tcp_info.rtt_var = json_object_get_double(obj);

    if (json_object_object_get_ex(jvalue, "BYTES_RCVD", &obj))
      flow->out_bytes = flow->tcp_info.rcvd_bytes =
          (u_int64_t)json_object_get_int64(obj);

    if (json_object_object_get_ex(jvalue, "BYTES_ACKED", &obj))
      flow->in_bytes = flow->tcp_info.sent_bytes =
          (u_int64_t)json_object_get_int64(obj);

    if (!flow->tcp_info_set)
      flow->tcp_info_set = true;
    flow->absolute_packet_octet_counters = true;
    ret = true;
          ...
          if (additional_o != NULL) {
            ...
            while (!json_object_iter_equal(&additional_it, &additional_itEnd)) {
              ...
              if ((additional_key != NULL) && (additional_value != NULL)) {
                // ntop->getTrace()->traceEvent(TRACE_NORMAL, "Additional field:
                // %s", additional_key);
...
#endif
        ntop->getTrace()->traceEvent(TRACE_DEBUG, "Not handled ZMQ field %u.%u",
                                     pen, key_id);
        add_to_additional_fields = true;
        break;
      } /* switch */
...
u_int8_t ZMQParserInterface::parseJSONFlow(const char *payload,
  ...
  if (f != NULL) {
    ...
    if (!once) {
      ntop->getTrace()->traceEvent(
          TRACE_WARNING, "Invalid message received: your nProbe sender is "
                         "outdated, data encrypted or invalid JSON?");
      ntop->getTrace()->traceEvent(
          TRACE_WARNING, "JSON Parse error [%s] payload size: %u payload: %s",
          json_tokener_error_desc(jerr), payload_size, payload);
    }
...
u_int8_t ZMQParserInterface::parseTLVFlow(const char *payload, int payload_size,
  ...
  if (ndpi_deserialize_get_format(&deserializer) !=
    ...
    if (!once) {
      ntop->getTrace()->traceEvent(
          TRACE_WARNING, "Invalid TLV message: the TLV generated by your probe "
                         "does not match the version supported "
                         "by ntopng, please update both the probe and ntopng "
...
bool ZMQParserInterface::parseContainerInfo(
  ...
  if (json_object_object_get_ex(jo, "ID", &obj))
    container_info->id = (char *)json_object_get_string(obj);

  if (json_object_object_get_ex(jo, "K8S", &obj)) {
    if (json_object_object_get_ex(obj, "POD", &obj2))
      container_info->data.k8s.pod = (char *)json_object_get_string(obj2);
    if (json_object_object_get_ex(obj, "NS", &obj2))
      container_info->data.k8s.ns = (char *)json_object_get_string(obj2);
  ...
  if (obj) {
    if (json_object_object_get_ex(obj, "NAME", &obj2))
      container_info->name = (char *)json_object_get_string(obj2);
  }

  // ntop->getTrace()->traceEvent(TRACE_NORMAL, "Container [id: %s] [%s]
  // [k8s.name: %s][k8s.pod: %s][k8s.ns: %s][docker.name: %s]",
  // 			       container_info->id ? container_info->id : "",
  // 			       container_info->data_type == container_info_data_type_k8s ?
  // "K8S" : container_info->data_type == container_info_data_type_docker ?
  // "DOCKER" : "UNKNOWN", 			       container_info->data_type ==
  // container_info_data_type_k8s && container_info->data.k8s.name ?
  // container_info->data.k8s.name : "", 			       container_info->data_type ==
  // container_info_data_type_k8s && container_info->data.k8s.pod ?
  // container_info->data.k8s.pod : "", 			       container_info->data_type ==
  // container_info_data_type_k8s && container_info->data.k8s.ns ?
  // container_info->data.k8s.ns : "", 			       container_info->data_type ==
  // container_info_data_type_docker && container_info->data.docker.name ?
  // container_info->data.docker.name : "");
...
u_int8_t ZMQParserInterface::parseCounter(const char *payload, int payload_size,
                                          u_int8_t source_id, void *data) {
  json_object *o;
  enum json_tokener_error jerr = json_tokener_success;
    ...
    while (!json_object_iter_equal(&it, &itEnd)) {
      ...
      if ((key != NULL) && (value != NULL)) {
        if (!strcmp(key, "deviceIP"))
          stats.deviceIP = ntohl(inet_addr(value));
          ...
          stats.ifPromiscuousMode = (!strcmp(value, "1")) ? true : false;
        else if (strlen(key) >= 9 &&
                 !strncmp(&key[strlen(key) - 9], "CONTAINER", 9)) {
...
u_int8_t
ZMQParserInterface::parseOptionFieldValueMap(json_object *const w) const {
  json_object *z;
