...
Flow::Flow(NetworkInterface *_iface, VLANid _vlanId,
           u_int16_t _observation_point_id, u_int8_t _protocol, Mac *_cli_mac,
           IpAddress *_cli_ip, u_int16_t _cli_port, Mac *_srv_mac,
           IpAddress *_srv_ip, u_int16_t _srv_port,
           const ICMPinfo *const _icmp_info, time_t _first_seen,
  ...
  last_update_time.tv_usec = 0, top_bytes_thpt = 0, top_pkts_thpt = 0;
  ...
  iface->findFlowHosts(_vlanId, _observation_point_id, _cli_mac, _cli_ip,
                       &cli_host, _srv_mac, _srv_ip, &srv_host);
  ...
  if (cli_host) {
    ...
    if (network_stats)
      network_stats->incNumFlows(last_seen, true);
  ...
  } else { /* Client host has not been allocated, let's keep the info in an
              IpAddress */
  ...
  if (srv_host) {
    ...
    if (network_stats)
      network_stats->incNumFlows(last_seen, false);
  ...
  } else { /* Server host has not been allocated, let's keep the info in an
              IpAddress */
  ...
  if (isBlacklistedClient()) {
    if (srv_host)
      srv_host->inc_num_blacklisted_flows(false);
...
Flow::~Flow() {
  ...
  if (viewFlowStats)
    delete (viewFlowStats);
  if (periodic_stats_update_partial)
    delete (periodic_stats_update_partial);
  if (last_db_dump.partial)
    delete (last_db_dump.partial);
  if (json_info)
  ...
  if (host_server_name)
    free(host_server_name);
  if (iec104)
    delete iec104;
  if (suspicious_dga_domain)
    free(suspicious_dga_domain);
  ...
  if (external_alert.json)
    json_object_put(external_alert.json);
  if (external_alert.source)
    free(external_alert.source);
}
...
void Flow::processDetectedProtocol(u_int8_t *payload, u_int16_t payload_len) {
  ...
  if (cli_h)
    cli_h->incnDPIFlows(stats_protocol);
  if (srv_h)
    srv_h->incnDPIFlows(stats_protocol);
  ...
  case NDPI_PROTOCOL_NTP:
    ...
    if (real_srv_h) {
      ...
      if (real_cli_h)
        real_cli_h->incNTPContactCardinality(real_srv_h);
    }
    break;
  ...
  case NDPI_PROTOCOL_MAIL_SMTP:
    if (srv_h) {
      srv_h->setSmtpServer();
      if (cli_h)
        cli_h->incSMTPContactCardinality(srv_h);
    }
    break;
  ...
  case NDPI_PROTOCOL_DNS:
    ...
    */
    if (srv_h) {
      srv_h->setDnsServer();
      if (cli_h)
        cli_h->incDNSContactCardinality(srv_h);
    }
    break;
...
void Flow::processDetectedProtocolData() {
  ...
  case NDPI_PROTOCOL_QUIC:
    if (ndpiFlow->host_server_name[0] != '\0') {
      ...
      if (cli_h)
        cli_h->incContactedService(ndpiFlow->host_server_name);
    }
    break;
  ...
  case NDPI_PROTOCOL_HTTP_PROXY:
    ...
    if (ndpiFlow->host_server_name[0] != '\0') {
      ...
      if ((doublecol = (char *)strchr((const char *)ndpiFlow->host_server_name,
                                      delimiter)) != NULL)
        doublecol[0] = '\0';
...
void Flow::processPacket(const struct pcap_pkthdr *h, const u_char *ip_packet,
  ...
  */
  ...
  if (isDNS())
    ...
    processIEC60870Packet((htons(src_port) == 2404) ? true : false, ip_packet,
                          ip_len, payload, payload_len,
                          (struct timeval *)&h->ts);
...
void Flow::processIEC60870Packet(bool tx_direction, const u_char *ip_packet,
                                 u_int16_t ip_len, const u_char *payload,
...
void Flow::setExtraDissectionCompleted() {
  ...
  if (!detection_completed) {
    ntop->getTrace()->traceEvent(
        TRACE_ERROR, "Bad internal status: setExtraDissectionCompleted called "
                     "before setDetectedProtocol");
    return;
  }
...
void Flow::hosts_periodic_stats_update(NetworkInterface *iface, Host *cli_host,
  ...
  if (cli_host && srv_host) {
    ...
    if (iface &&
      ...
      /* Note: vl will never be null as we're in a flow with that vlan. Hence,
         it is guaranteed that at least
         two hosts exists for that vlan and that any purge attempt will be
         prevented. */
#ifdef VLAN_DEBUG
  ...
  case IPPROTO_ICMP:
    if (iface) {
      if (partial->get_cli2srv_packets())
        iface->incICMPStats(false /* icmp v4 */, partial->get_cli2srv_packets(),
                            protos.icmp.cli2srv.icmp_type,
                            protos.icmp.cli2srv.icmp_code, true);

      if (partial->get_srv2cli_packets())
        iface->incICMPStats(false /* icmp v4 */, partial->get_srv2cli_packets(),
  ...
  case IPPROTO_ICMPV6:
    if (iface) {
      if (partial->get_cli2srv_packets())
        iface->incICMPStats(true /* icmp v6 */, partial->get_cli2srv_packets(),
                            protos.icmp.cli2srv.icmp_type,
                            protos.icmp.cli2srv.icmp_code, true);

      if (partial->get_srv2cli_packets())
        iface->incICMPStats(true /* icmp v6 */, partial->get_srv2cli_packets(),
                            protos.icmp.srv2cli.icmp_type,
                            protos.icmp.srv2cli.icmp_code, true);
    }
  ...
  case NDPI_PROTOCOL_HTTP_PROXY:
    if (srv_host) {
      if (!Utils::isIPAddress(host_server_name) &&
          hasRisk(NDPI_HTTP_NUMERIC_IP_HOST)) {
  ...
  case NDPI_PROTOCOL_MDNS:
    if (cli_host) {
      ...
      if (protos.mdns.name)
        cli_host->offlineSetMDNSName(protos.mdns.name);
      if (protos.mdns.name_txt)
  ...
  case NDPI_PROTOCOL_IP_ICMPV6:
    ...
    if (first_partial && icmp_info) {
      if (icmp_info->isPortUnreachable()) { // Port unreachable icmpv6/icmpv4
        ...
        if (srv_host)
          srv_host->incNumUnreachableFlows(true /* as server */);
        if (cli_host)
          cli_host->incNumUnreachableFlows(false /* as client */);
...
void Flow::updateThroughputStats(float tdiff_msec, u_int32_t diff_sent_packets,
  ...
  if (tdiff_msec == 0)
    ...
    if (bytes_msec < 0)
      bytes_msec = 0;
    if (bytes_msec_cli2srv < 0)
      bytes_msec_cli2srv = 0;
    if (bytes_msec_srv2cli < 0)
      bytes_msec_srv2cli = 0;
    if (goodput_bytes_msec < 0)
      goodput_bytes_msec = 0;
    if (goodput_bytes_msec_cli2srv < 0)
      goodput_bytes_msec_cli2srv = 0;
    if (goodput_bytes_msec_srv2cli < 0)
      goodput_bytes_msec_srv2cli = 0;
    ...
    if ((bytes_msec > 0) || iface->isPacketInterface()) {
      ...
      if ((get_goodput_bytes_cli2srv() + get_goodput_bytes_srv2cli()) > 0) {
        ...
        ntop->getTrace()->traceEvent(
            TRACE_NORMAL,
            "%s [Goodput long/mid/short %.3f/%.3f/%.3f][ratio: "
            "%s][goodput/thpt: %.3f]",
      ...
      // pps
      float pkts_msec_cli2srv =
          ((float)(diff_sent_packets * 1000)) / tdiff_msec;
      float pkts_msec_srv2cli =
          ((float)(diff_rcvd_packets * 1000)) / tdiff_msec;
      float pkts_msec = pkts_msec_cli2srv + pkts_msec_srv2cli;

      /* Just to be safe */
      if (pkts_msec < 0)
        pkts_msec = 0;
      if (pkts_msec_cli2srv < 0)
        pkts_msec_cli2srv = 0;
      if (pkts_msec_srv2cli < 0)
        pkts_msec_srv2cli = 0;
      ...
      if (top_pkts_thpt < get_pkts_thpt())
        top_pkts_thpt = get_pkts_thpt();

#if DEBUG_TREND
      u_int64_t diff_pkts = diff_sent_packets + diff_rcvd_packets;
      ntop->getTrace()->traceEvent(
          TRACE_NORMAL,
          "[msec: %.1f][tdiff: %f][pkts: %lu][pkts_thpt: %.2f pps]", pkts_msec,
          tdiff_msec, diff_pkts, get_pkts_thpt());
#endif
    }
  }
}
...
void Flow::periodic_stats_update(const struct timeval *tv) {
  ...
  /* Non-Packet interfaces (e.g., ZMQ) have flow throughput stats updated as
     soon as the flow is received. This makes throughput more precise as it is
     averaged on a timespan which is last-first switched. */
...
bool Flow::equal(const IpAddress *_cli_ip, const IpAddress *_srv_ip,
                 u_int16_t _cli_port, u_int16_t _srv_port, VLANid _vlanId,
                 u_int16_t _observation_point_id, u_int8_t _protocol,
...
void Flow::lua(lua_State *vm, AddressTree *ptree, DetailsLevel details_level,
  ...
  if (ptree) {
    if (src_ip)
      src_match = src_ip->match(ptree);
    if (dst_ip)
      dst_match = dst_ip->match(ptree);
  ...
  if (details_level >= details_high) {
    ...
    if (vrfId)
      lua_push_uint64_table_entry(vm, "vrfId", vrfId);
    ...
    if (!mask_flow) {
      ...
      if (bt_hash)
        lua_push_str_table_entry(vm, "bittorrent_hash", bt_hash);
    ...
    if (cli_ebpf)
      cli_ebpf->lua(vm, true);
    if (srv_ebpf)
      srv_ebpf->lua(vm, false);
    ...
    lua_push_str_table_entry(vm, "confidence",
                             ndpi_confidence_get_name(confidence));
...
u_int32_t Flow::key() {
  ...
  if (get_cli_ip_addr())
    k += get_cli_ip_addr()->key();
  if (get_srv_ip_addr())
    k += get_srv_ip_addr()->key();
  if (icmp_info)
    k += icmp_info->key();
...
u_int32_t Flow::key(Host *_cli, u_int16_t _cli_port, Host *_srv,
                    u_int16_t _srv_port, VLANid _vlan_id,
...
u_int Flow::get_hash_entry_id() const { return hash_entry_id; };
...
bool Flow::is_hash_entry_state_idle_transition_ready() {
  ...
  if (iface->getIfType() == interface_type_ZMQ) {
    ...
    if (protocol == IPPROTO_TCP) {
      ...
      if ((
        ...
        /* ntop->getTrace()->traceEvent(TRACE_NORMAL, "[TCP] Early flow
         * expire"); */
        ret = true;
      }
    }
  ...
  if (ret &&
      ((iface->getTimeLastPktRcvd() - flowCreationTime) < 10 /* sec */)) {
    /*
...
json_object *Flow::flow2JSON() {
  ...
  if (ntop->getPrefs()->do_dump_flows_on_es()) {
    ...
    */
    ...
    // MAC addresses are set only when dumping to ES to optimize space
    // consumption
    ...
    if (srv_host && srv_host->getMac() && !srv_host->getMac()->isNull())
      json_object_object_add(
          my_object,
          Utils::jsonLabel(OUT_DST_MAC, "OUT_DST_MAC", jsonbuf,
                           sizeof(jsonbuf)),
          json_object_new_string(Utils::formatMac(
  ...
  if (ntop->getPrefs()->do_dump_flows_on_syslog()) {
    ...
    if (isSSH() && protos.ssh.hassh.client_hash)
      json_object_object_add(
          my_object,
          Utils::jsonLabel(HASSHC_HASH, "HASSHC_HASH", jsonbuf,
                           sizeof(jsonbuf)),
  ...
  if (cli_ip) {
    ...
    json_object_object_add(
        my_object,
        Utils::jsonLabel(SRC_ADDR_LOCAL, "SRC_ADDR_LOCAL", jsonbuf,
                         sizeof(jsonbuf)),
  ...
  if (srv_ip) {
    ...
    json_object_object_add(
        my_object,
        Utils::jsonLabel(DST_ADDR_LOCAL, "DST_ADDR_LOCAL", jsonbuf,
                         sizeof(jsonbuf)),
  ...
  json_object_object_add(
      my_object,
      Utils::jsonLabel(FIRST_SWITCHED, "FIRST_SWITCHED", jsonbuf,
                       sizeof(jsonbuf)),
      json_object_new_int((u_int32_t)get_partial_first_seen()));
  json_object_object_add(
      my_object,
      Utils::jsonLabel(LAST_SWITCHED, "LAST_SWITCHED", jsonbuf,
                       sizeof(jsonbuf)),
  ...
  if (protocol == IPPROTO_TCP) {
    json_object_object_add(my_object,
                           Utils::jsonLabel(CLIENT_NW_LATENCY_MS,
                                            "CLIENT_NW_LATENCY_MS", jsonbuf,
                                            sizeof(jsonbuf)),
                           json_object_new_double(toMs(&clientNwLatency)));
    json_object_object_add(my_object,
                           Utils::jsonLabel(SERVER_NW_LATENCY_MS,
                                            "SERVER_NW_LATENCY_MS", jsonbuf,
                                            sizeof(jsonbuf)),
  ...
  if (ntop->getPrefs() && ntop->getPrefs()->get_instance_name())
    json_object_object_add(
        my_object, "NTOPNG_INSTANCE_NAME",
        json_object_new_string(ntop->getPrefs()->get_instance_name()));
  if (iface && iface->get_name())
    json_object_object_add(my_object, "INTERFACE",
                           json_object_new_string(iface->get_name()));

  if (isDNS() && protos.dns.last_query)
    json_object_object_add(my_object, "DNS_QUERY",
  ...
  if (flow_device.device_ip)
    json_object_object_add(my_object, "EXPORTER_IPV4_ADDRESS",
                           json_object_new_string(intoaV4(flow_device.device_ip,
                                                          buf, sizeof(buf))));

  if (bt_hash)
    json_object_object_add(my_object, "BITTORRENT_HASH",
                           json_object_new_string(bt_hash));

  if (isTLS() && protos.tls.client_requested_server_name)
    json_object_object_add(
        my_object, "TLS_SERVER_NAME",
  ...
  if (cli_ebpf)
    cli_ebpf->getJSONObject(my_object, true);
  if (srv_ebpf)
    srv_ebpf->getJSONObject(my_object, false);
  ...
  if (ntop->getPrefs()->do_dump_extended_json()) {
    ...
    if (cli_ip) {
      if (cli_ip->isIPv4()) {
        json_object_object_add(my_object,
                               Utils::jsonLabel(IP_PROTOCOL_VERSION,
                                                "IP_PROTOCOL_VERSION", jsonbuf,
                                                sizeof(jsonbuf)),
    ...
    json_object_object_add(my_object, "INTERFACE_ID",
...
void Flow::alert2JSON(FlowAlert *alert, ndpi_serializer *s) {
  ...
  ndpi_serialize_string_int32(s, "ifid",
                              iface->isViewed() ? iface->viewedBy()->get_id()
                                                : iface->get_id());
  ...
  if (isFlowAlerted()) {
    ...
    if (!getInterface()
        ...
        || (getViewInterfaceFlowStats() && getViewInterfaceFlowStats()
                                               ...
                                               ->get_is_flow_alerted())) {
      if (cli_u)
        cli_u->decNumAlertedFlows(true /* As client */);
      if (srv_u)
        srv_u->decNumAlertedFlows(false /* As server */);
    }
...
void Flow::housekeep(time_t t) {
  ...
  case hash_entry_state_flow_notyetdetected:
    /*
      ...
      Giveup is handled differently, depending on whether we are processing pcap
      files or not:
      - When not reading from pcap files, before giving up, we wait at least 5
      seconds since the last flow packet
      - When reading from pcap files, we wait until all the packets in the file
      have been processed before ending the dissection
    */
  ...
  case hash_entry_state_active:
    ...
    /*
      ...
      In case of interfaces processing pcap files, if the processing of the pcap
      file is completed, i.e. read_from_pcap_dump_done() is true, then there
      will be no more packets so the flow swap won't take place. For this
      reason, the callbacks are executed on the original flow that should have
      been swapped but actually it is not.
     */
  ...
  case hash_entry_state_idle:
    ...
    if (is_swap_requested() &&
        !is_swap_done()) /* Swap requested but never performed (no more packets
                            seen) */
      iface->execProtocolDetectedChecks(this);
...
void Flow::incStats(bool cli2srv_direction, u_int pkt_len, u_int8_t *payload,
  ...
  */
  ...
  updatePacketStats(cli2srv_direction ? getCli2SrvIATStats()
                                      : getSrv2CliIATStats(),
                    when, update_iat);
  ...
  if (cli2srv_direction) {
    ...
    if (cli_host)
      cli_host->incSentStats(1, pkt_len);
    if (srv_host)
      srv_host->incRecvStats(1, pkt_len);
  } else {
    ip_stats_d2s.pktFrag += is_fragment;
    if (cli_host)
      cli_host->incRecvStats(1, pkt_len);
    if (srv_host)
      srv_host->incSentStats(1, pkt_len);
  }
...
void Flow::updateInterfaceLocalStats(bool src2dst_direction, u_int num_pkts,
                                     u_int pkt_len) {
  const IpAddress *from =
      src2dst_direction ? get_cli_ip_addr() : get_srv_ip_addr();
  const IpAddress *to =
      src2dst_direction ? get_srv_ip_addr() : get_cli_ip_addr();
  int16_t from_id = 0;
  int16_t to_id = 0;
...
void Flow::addFlowStats(bool new_flow, bool cli2srv_direction, u_int in_pkts,
  ...
  if (new_flow)
    ...
    /* Average of the latest update, that is between the new and the previous
     * last_seen */
...
<  ntop->getTrace()->traceEvent(TRACE_NORMAL, "[first: %u][last: %u][get_last_seen: %u][%u][%u][in_bytes: %u][out_bytes: %u][bytes : %u][thpt: %.2f]",
  ...
  */
  if (!new_flow && thp_delta_time <= 5) {
    /*
       If here, delta time is too small to enable meaningful throughput
       calculations using only bytes/packets delta. In this case, totals are
       used and averaged using the overall flow lifetime.
     */
    if (cli2srv_direction)
      updateThroughputStats(get_duration() * 1000, get_packets_cli2srv(),
                            get_bytes_cli2srv(), 0, get_packets_srv2cli(),
                            get_bytes_srv2cli(), 0);
    else
      updateThroughputStats(get_duration() * 1000, get_packets_srv2cli(),
                            get_bytes_srv2cli(), 0, get_packets_cli2srv(),
                            get_bytes_cli2srv(), 0);
  } else {
    /*
     ...
     */
    if (cli2srv_direction)
      updateThroughputStats(thp_delta_time * 1000, in_pkts, in_bytes, 0,
                            out_pkts, out_bytes, 0);
    else
      updateThroughputStats(thp_delta_time * 1000, out_pkts, out_bytes, 0,
                            in_pkts, in_bytes, 0);
  }
}
  ...
  if (cumulative_flags) {
    if (!twh_over) {
      if ((src2dst_tcp_flags & (TH_SYN | TH_ACK)) == (TH_SYN | TH_ACK) &&
        ...
        twh_ok = twh_over = true, iface->getTcpFlowStats()->incEstablished();
    }
  } else {
    if (!twh_over) {
      if (flags_3wh == TH_SYN) {
...
void Flow::incTcpBadStats(bool src2dst_direction, Host *cli, Host *srv,
  ...
  if (iface) {
    ...
    if (keep_alive_pkts)
      iface->incKeepAlivePkts(keep_alive_pkts);
  }
...
void Flow::updateTcpSeqNum(const struct bpf_timeval *when, u_int32_t seq_num,
  ...
  if (src2dst_direction) {
    ...
    if (window > 0)
      srv2cli_window = window; /* Note the window is reverted */
    ...
    if (window > 0)
      cli2srv_window = window; /* Note the window is reverted */
...
void Flow::setBittorrentHash(char *hash) {
  ...
  if (n > 0)
    bt_hash = strdup(bittorrent_hash);
}
...
void Flow::updateHTTP(ParsedFlow *zflow) {
  if (isHTTP()) {
    ...
    while (ret_code > 9)
      ret_code /= 10; /* Take the first digit */
    switch (ret_code) {
    case 1:
          ...
          if (txt_len < data_len) {
            ...
            if (txt_len > 0) {
              ...
              if (strncmp(txt_buf, "ssid=", 5) == 0) {
                if (!protos.mdns.ssid)
                  protos.mdns.ssid = strdup(&txt_buf[5]);
  ...
  if (strncmp(payload, "NOTIFY", 6) == 0) {
    ...
    for (; 0 < payload_len - 9 /* strlen("Location:") */;
      ...
      if (strncasecmp(payload, "Location:", 9)) {
        ...
        if (!protos.ssdp.location)
          protos.ssdp.location = strdup(url);
        break;
      }
    }
  }
  ...
  if (cli_host && srv_host) {
    if (src2dst_direction) {
      ...
      *egress_shaper = cli_host->get_egress_shaper(ndpiDetectedProtocol);
      ...
      if (*ingress_shaper)
        srv2cli_in = (*ingress_shaper)->get_shaper_id();
      if (*egress_shaper)
        cli2srv_out = (*egress_shaper)->get_shaper_id();
      ...
      *egress_shaper = srv_host->get_egress_shaper(ndpiDetectedProtocol);
      ...
      if (*ingress_shaper)
        cli2srv_in = (*ingress_shaper)->get_shaper_id();
      if (*egress_shaper)
        srv2cli_out = (*egress_shaper)->get_shaper_id();
    }
...
void Flow::setParsedeBPFInfo(const ParsedeBPF *const ebpf,
                             bool src2dst_direction) {
  ...
  if (cur && cur->container_info_set) {
    ...
    if (cur->container_info.data_type == container_info_data_type_k8s &&
        !iface->hasSeenPods() && cur->container_info.data.k8s.pod)
      iface->setSeenPods();
  }
...
void Flow::lua_get_bytes(lua_State *vm) const {
  ...
  lua_push_uint64_table_entry(vm, "bytes.last",
                              get_current_bytes_cli2srv() +
                                  get_current_bytes_srv2cli());
...
void Flow::lua_get_dir_traffic(lua_State *vm, bool cli2srv) const {
  ...
  lua_push_uint64_table_entry(vm, cli2srv ? "cli2srv.last" : "srv2cli.last",
                              cli2srv ? get_current_bytes_cli2srv()
                                      : get_current_bytes_srv2cli());
  ...
  if (h) {
    ...
    if (!mask_host) {
      ...
      lua_push_bool_table_entry(vm,
                                client ? "cli.broadcast_domain_host"
                                       : "srv.broadcast_domain_host",
...
void Flow::lua_get_min_info(lua_State *vm) {
  ...
  if (info)
    lua_push_str_table_entry(vm, "info", info);
}
...
void Flow::getInfo(ndpi_serializer *serializer) {
  ...
  ndpi_serialize_string_uint64(serializer, "srv2cli.packets",
                               get_packets_srv2cli());
  if (info)
    ndpi_serialize_string_string(serializer, "info", info);
}
...
void Flow::setNormalToAlertedCounters() {
  ...
  if (cli_h)
    cli_h->incNumAlertedFlows(true /* As client */), cli_h->incTotalAlerts();

  if (srv_h)
    srv_h->incNumAlertedFlows(false /* As server */), srv_h->incTotalAlerts();
...
#ifdef DEBUG_SCORE
  ntop->getTrace()->traceEvent(TRACE_NORMAL, "Set alert score: %u (%u/%u)",
                               flow_inc, cli_inc, srv_inc);
#endif
  ...
  if (!(get_cli_ip_addr()->isNonEmptyUnicastAddress() &&
        !get_srv_ip_addr()
             ->isNonEmptyUnicastAddress()) /* Everything that is NOT
                                              unicast-to-non-unicast needs to be
                                              checked */
