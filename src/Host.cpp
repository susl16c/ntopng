...
Host::Host(NetworkInterface *_iface, char *ipAddress, VLANid _vlanId,
           u_int16_t observation_point_id)
    : GenericHashEntry(_iface), HostAlertableEntity(_iface, alert_entity_host),
      Score(_iface), HostChecksStatus() {
  ip.set(ipAddress);
...
Host::Host(NetworkInterface *_iface, Mac *_mac, VLANid _vlanId,
...
Host::~Host() {
  ...
  // ntop->getTrace()->traceEvent(TRACE_NORMAL, "Deleting %s (%s)", k, localHost
  // ? "local": "remote");
  ...
  if (syn_flood.attacker_counter)
    delete syn_flood.attacker_counter;
  if (syn_flood.victim_counter)
    delete syn_flood.victim_counter;
  if (flow_flood.attacker_counter)
    delete flow_flood.attacker_counter;
  if (flow_flood.victim_counter)
    delete flow_flood.victim_counter;
  if (icmp_flood.attacker_counter)
    delete icmp_flood.attacker_counter;
  if (icmp_flood.victim_counter)
    delete icmp_flood.victim_counter;
...
u_int16_t Host::incScoreValue(u_int16_t score_incr,
   ...
   * host */
  if (as)
    as->incScoreValue(score_incr, score_category, as_client);
  if (vlan)
    vlan->incScoreValue(score_incr, score_category, as_client);
  if (country)
    country->incScoreValue(score_incr, score_category, as_client);
  if (obs_point)
    obs_point->incScoreValue(score_incr, score_category, as_client);
  if (ns)
    ns->incScoreValue(score_incr, score_category, as_client);
  if (iface)
    iface->incScoreValue(score_incr, as_client);
...
u_int16_t Host::decScoreValue(u_int16_t score_decr,
  ...
  if (as)
    as->decScoreValue(score_decr, score_category, as_client);
  if (vlan)
    vlan->decScoreValue(score_decr, score_category, as_client);
  if (country)
    country->decScoreValue(score_decr, score_category, as_client);
  if (obs_point)
    obs_point->decScoreValue(score_decr, score_category, as_client);
  if (ns)
    ns->decScoreValue(score_decr, score_category, as_client);
  if (iface)
    iface->decScoreValue(score_decr, as_client);
...
void Host::initialize(Mac *_mac, VLANid _vlanId,
                      u_int16_t observation_point_id) {
  char buf[64];
  u_int16_t masked_vlanId =
      filterVLANid(_vlanId); /* Cleanup any possible junk */
  ...
  // readStats(); - Commented as if put here it's too early and the key is not
  // yet set
  ...
  if ((vlan = iface->getVLAN(masked_vlanId, true, true /* Inline call */)) !=
      NULL)
    vlan->incUses();
  ...
  if (ip.getVersion() /* IP is set */) {
    ...
    if ((obs_point = iface->getObsPoint(observation_point_id,
...
void Host::lua_get_names(lua_State *const vm, char *const buf,
  ...
  if (buf[0])
    lua_push_str_table_entry(vm, "mdns", buf);

  getMDNSTXTName(buf, buf_size);
  if (buf[0])
    lua_push_str_table_entry(vm, "mdns_txt", buf);

  getResolvedName(buf, buf_size);
  if (buf[0])
    lua_push_str_table_entry(vm, "resolved", buf);

  getNetbiosName(buf, buf_size);
  if (buf[0])
    lua_push_str_table_entry(vm, "netbios", buf);

  getTLSName(buf, buf_size);
  if (buf[0])
    lua_push_str_table_entry(vm, "tls", buf);

  getHTTPName(buf, buf_size);
  if (buf[0])
    lua_push_str_table_entry(vm, "http", buf);

  if (isBroadcastDomainHost() && cur_mac) {
    cur_mac->getDHCPName(buf, buf_size);
    if (buf[0])
      lua_push_str_table_entry(vm, "dhcp", buf);
  }
  ...
  if (host_details) {
    ...
    */
    ...
    if (strlen(buf) == 0 || strcmp(buf, ipaddr) == 0) {
      /* We resolve immediately the IP address by queueing on the top of address
       * queue */
...
void Host::periodic_stats_update(const struct timeval *tv) {
  ...
  if (!os || os->get_os_type() != cur_os_type)
...
void Host::serialize(json_object *my_object, DetailsLevel details_level) {
  ...
  /* The value below is handled by reading dumps on disk as otherwise the string
   * will be too long */
  // json_object_object_add(my_object, "activityStats",
  // activityStats.getJSONObject());
}
...
void Host::refreshDisabledAlerts() {
  ...
  if (alert_exclusions &&
      alert_exclusions->checkChange(&disabled_alerts_tstamp)) {
