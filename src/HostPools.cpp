...
HostPools::~HostPools() {
  ...
  if (children_safe)
    free(children_safe);
  if (forge_global_dns)
    free(forge_global_dns);
  if (routing_policy_id)
    free(routing_policy_id);
  if (pool_shaper)
    free(pool_shaper);
  if (schedule_bitmap)
...
void HostPools::lua(lua_State *vm) {
  ...
  for (int i = 0; i < MAX_NUM_HOST_POOLS; i++) {
    if ((cur_hosts = getNumPoolHosts(i)))
      hosts += cur_hosts;
    if ((cur_l2 = getNumPoolL2Devices(i)))
      l2_devices += cur_l2;
...
void HostPools::reloadPools() {
  ...
  if (new_stats[0])
    new_stats[0]->updateName(DEFAULT_POOL_NAME);
...
bool HostPools::findIpPool(IpAddress *ip, VLANid vlan_id, u_int16_t *found_pool,
