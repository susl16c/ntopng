...
NetworkDiscovery::NetworkDiscovery(NetworkInterface *_iface) {
...
#endif
  {
    ...
    const char *bpfFilter =
        "arp && arp[6:2] = 2"; // arp[x:y] - from byte 6 for 2 bytes (arp.opcode
                               // == 2 -> reply)
...
u_int16_t NetworkDiscovery::in_cksum(u_int8_t *buf, u_int16_t buf_len,
                                     u_int32_t sum) {
  u_int i;
  ...
  if (mdns_vm) {
    ...
    */
    if (mdns_vm) {
      ...
      char *ip =
          Utils::intoaV4(ntohl(src_ip_nw_byte_order), ipbuf, sizeof(ipbuf));
...
*/
void NetworkDiscovery::sendArpNetwork(void *data, u_int32_t netp,
                                      u_int32_t maskp, u_int32_t sender_ip) {
    ...
    for (host_ip = first_ip; host_ip < last_ip; host_ip++) {
      ...
      if (reply) {
        ...
        if (mdns_sock != -1) {
          ...
          if (sendto(mdns_sock, arp_d->mdnsbuf, arp_d->dns_query_len, 0,
                     (struct sockaddr *)mdns_dest,
                     sizeof(struct sockaddr_in)) < 0)
            ntop->getTrace()->traceEvent(TRACE_ERROR, "MDNS Send error [%d/%s]",
...
void NetworkDiscovery::arpScan(lua_State *vm) {
  ...
  if (!pd || !iface->getInterfaceNetworks())
    return;
  ...
  if ((mdns_sock = socket(AF_INET, SOCK_DGRAM, 0)) == -1)
    ...
    for (dns_query_len = 0; anyservices[dns_query_len] != '\0';
         dns_query_len++) {
    ...
    queries[dns_query_len++] = 0x0C; /* PTR */
    queries[dns_query_len++] = 0x00;
...
u_int16_t NetworkDiscovery::buildMDNSDiscoveryDatagram(const char *query,
  ...
  struct ndpi_iphdr *iph =
      (struct ndpi_iphdr *)(&pbuf[sizeof(struct ndpi_ethhdr)]);
  struct ndpi_udphdr *udph =
      (struct ndpi_udphdr
           *)&pbuf[sizeof(struct ndpi_ethhdr) + sizeof(struct ndpi_iphdr)];
  ...
  data = &pbuf[sizeof(struct ndpi_ethhdr) + sizeof(struct ndpi_iphdr) +
  ...
  iph->tos = 0;
  iph->tot_len =
      sizeof(struct ndpi_iphdr) + sizeof(struct ndpi_udphdr) + dns_query_len;
  ...
  udph->check = wrapsum(in_cksum(
      (unsigned char *)udph, sizeof(struct ndpi_udphdr),
      in_cksum((unsigned char *)data, dns_query_len,
               in_cksum((unsigned char *)&iph->saddr, 2 * sizeof(iph->saddr),
...
void _dissectMDNS(u_char *buf, u_int buf_len, char *out, u_int out_len) {
  ...
  out[0] = '\0';
  if (buf_len < sizeof(struct ndpi_dns_packet_header))
    return;

  num_queries = ntohs(dns_h->num_queries),
  num_answers = ntohs(dns_h->num_answers);
  ...
  if (num_queries > 0) {
    ...
    for (i = 0; (i < num_queries) && (offset < (u_int)buf_len); i++) {
      for (idx = 0, dissected_ptr = false;
           (offset < buf_len) && (idx < sizeof(rspbuf) - 1); idx++, offset++) {
        if (buf[offset] == 0) {
          if (dissected_ptr)
            offset--;
          break;
...
void NetworkDiscovery::dissectMDNS(u_char *buf, u_int buf_len, char *out,
                                   u_int out_len) {
...
void NetworkDiscovery::discover(lua_State *vm, u_int timeout) {
  ...
  /*
    ...
    given type urn:schemas-upnp-org:service:serviceType- version: locate service
    of a given type
  */
  ...
  if (sendto(udp_sock, msg, strlen(msg), 0, (struct sockaddr *)&sin,
             sizeof(struct sockaddr_in)) < 0)
    ntop->getTrace()->traceEvent(TRACE_ERROR, "Send error [%d/%s]", errno,
                                 strerror(errno));
  ...
  /* MDNS */
  {
    ...
    const char *query_list[] = {
        "_sftp-ssh._tcp.local",   "_homekit._tcp.local.",   "_smb._tcp.local",
        "_afpovertcp._tcp.local", "_ssh._tcp.local",        "_nfs._tcp.local",
        "_airplay._tcp.local",    "_googlecast._tcp.local", NULL};
    int i;
    ...
    for (i = 0; query_list[i] != NULL; i++) {
      u_int16_t len = buildMDNSDiscoveryDatagram(query_list[i], sender_ip,
                                                 sender_mac, msg, sizeof(msg));
  ...
  while (select(udp_sock + 1, &fdset, NULL, NULL, &tv) > 0) {
    ...
    if (len > 0) {
      char src[32],
          *host = Utils::intoaV4(ntohl(from.sin_addr.s_addr), src, sizeof(src));
      char *line, *tmp;
      ...
      if (line) {
        while ((line = strtok_r(NULL, "\r", &tmp)) != NULL) {
          ...
          if (strncasecmp(line, "Location:", 9) == 0) {
            // ntop->getTrace()->traceEvent(TRACE_NORMAL, "[%s] %s", host,
            // &line[10]);
...
void NetworkDiscovery::discoverDHCP(u_char *mac) {
  ...
  u_char dhcp_discover[] = {
      0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x01, 0x02, 0x03, 0x04, 0x05,
      0x06, /* sender MAC */
      0x08, 0x00,
      /* IPv4 */
      0x45, 0x10, 0x01, 0x1e, 0xff, 0xff, 0x00, 0x00, 0x10, 0x11, 0xa9,
      0xc0, /* IP checksum */
      0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff,
      /* UDP */
      0x00, 0x44, 0x00, 0x43, 0x01, 0x0a, 0x00, 0x00,
      /* DHCP */
      0x01, 0x01, 0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
      0x06, /* sender MAC */
      0x06, 0xfb, 0x6e, 0xef, 0xfe, 0x7f, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      ...
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x63, 0x82,
      0x53, 0x63, 0x35, 0x01, 0x01, 0x32, 0x04, 0x01, 0x02, 0x03,
      0x04, /* Requested IP address (1.2.3.4) */
  ...
  if (pcap_sendpacket(pd, dhcp_discover, sizeof(dhcp_discover)) == -1)
    ntop->getTrace()->traceEvent(TRACE_NORMAL,
                                 "Error while sending DHCP discovery");
}
